#!/usr/bin/env python3
"""
Синхронізатор статистики ВФД.

Читає дві опубліковані Google-таблиці (CSV, без авторизації) і збирає
єдиний data.json для сайту:
  1. "UA darts stat (учасники)" — турніри, учасники, посилання (Nakka/протоколи)
  2. "Призери етапів кубків ВФД" — призери по роках/етапах + підсумковий залік

Запускається щодня через GitHub Actions (.github/workflows/sync.yml).
"""

import csv
import io
import json
import re
import time
import urllib.request
from datetime import datetime, timezone

TOURNAMENTS_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTZxNlB-yHQDjWX3Y_n4GCUL_4sY5oLcLeW9rR_MI5zlm2p0YqZmHUUXw07bLw1YTiUg4Ar6bRbn_Dd/pub?output=csv&gid=0"
CHAMPIONS_CSV_URL = "https://docs.google.com/spreadsheets/d/1QWz4s3O0hfLZ5ko8V8fn8Xuy59sa2zfglqyDixnFLNk/export?format=csv&gid=1583448563"
BIO_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSSKkG1BP7xihgTkLgBg6Xju_iUDyNeG65Zn20v8hR6g8WS8m2Rf-DhUHgZueHZVD3FKhmjhJ1OpnsN/pub?output=csv"
PRIZES_MEN_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vR5IoUV8U550qzdDKkLxenpx2LUYMQ8Uccqf9ZdkyP7ruIqdoPt_tX-hQWKhQOnTGc6HG6jiPQmQEuA/pub?output=csv&gid=0"
PRIZES_WOMEN_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vR5IoUV8U550qzdDKkLxenpx2LUYMQ8Uccqf9ZdkyP7ruIqdoPt_tX-hQWKhQOnTGc6HG6jiPQmQEuA/pub?output=csv&gid=109502045"
RATINGS_SOURCES_PATH = "ratings_sources.json"
RATING_HISTORY_PATH = "rating_history.json"
CURRENT_RATING_MEN_URL = "https://docs.google.com/spreadsheets/d/13BTy_ZDFgS1sz5iZ7dKHr1FXnVsbS5hzAS08R91n3as/export?format=csv&gid=583085584"
CURRENT_RATING_WOMEN_URL = "https://docs.google.com/spreadsheets/d/13BTy_ZDFgS1sz5iZ7dKHr1FXnVsbS5hzAS08R91n3as/export?format=csv&gid=964362865"
NAKKA_API_BASE = "https://push.n01darts.com/api/v1"
YOUTUBE_CHANNEL_ID = "UClyHuQB21ETTD7Q6V0cKmXQ"  # Ukrainian Darts Federation
TELEGRAM_CHANNEL = "fullbull"  # інформаційний партнер ВФД

# tdid турнірів, дані Nakka по яких явно биті (незрозумілі символи, аномальна
# кількість 180-ок тощо) — призерів з них показуємо як завжди, але в жодну
# статистику (Рекорди, середні, H2H) ці турніри не потрапляють.
EXCLUDED_STATS_TDIDS = {
    "t_tjq4_1053",  # UDL STAGE 3, Ужгород, 13.04.2024 — биті дані 180-ок
}

OUTPUT_PATH = "data.json"


def fetch_csv(url):
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (vfd-darts-sync)"})
    with urllib.request.urlopen(req, timeout=30) as resp:
        raw = resp.read().decode("utf-8-sig")
    return list(csv.reader(io.StringIO(raw)))


def parse_num(s):
    s = (s or "").strip()
    if s in ("", "-"):
        return None
    if "," in s and "." in s:
        # Обидва символи присутні — кома це роздільник ТИСЯЧ (напр. "1,049.60"),
        # а крапка — десятковий роздільник. Просто прибираємо коми.
        s = s.replace(",", "")
    else:
        # Лише кома — це десятковий роздільник (європейський запис, напр. "49,60").
        s = s.replace(",", ".")
    try:
        return float(s) if "." in s else int(s)
    except ValueError:
        return None


def classify_link(url):
    if not url:
        return None
    if "n01darts.com" in url:
        return "nakka"
    if "docs.google.com/document" in url:
        return "protocol_doc"
    if "docs.google.com/spreadsheets" in url:
        return "protocol_sheet"
    if "open.udf.in.ua" in url:
        return "udf_online"
    return "other"


# ---------------------------------------------------------------------------
# 1) "UA darts stat (учасники)"
#    Колонки: A total, B men, C menAvg, D women, E womenAvg, F name, G org,
#    H format, I date, J city, (K пусто), L..P — посилання (додані Apps Script)
# ---------------------------------------------------------------------------
def parse_tournaments(rows):
    tournaments = []
    for row in rows:
        if not row or len(row) < 10:
            continue
        row = row + [""] * (16 - len(row))
        total, men, men_avg, women, women_avg, name, org, fmt, date, city = row[:10]
        chu_category = row[10].strip() if len(row) > 10 else ""
        link_men, link_menavg, link_women, link_womenavg, link_tour = row[11:16]

        name = name.strip()
        date = date.strip()
        # skip header / blank rows: a real row always has a name and a dd.mm.yyyy date
        if not name or not re.match(r"^\d{2}\.\d{2}\.\d{4}$", date):
            continue

        org = org.strip()
        is_udl = org in ("УДЛ", "ЗУДЛ")
        org_norm = "УДЛ/ЗУДЛ" if is_udl else org

        def link_obj(u):
            u = (u or "").strip()
            return {"url": u, "type": classify_link(u)} if u else None

        tournaments.append({
            "total": parse_num(total), "men": parse_num(men), "menAvg": parse_num(men_avg),
            "women": parse_num(women), "womenAvg": parse_num(women_avg),
            "name": name, "org": org_norm, "format": fmt.strip(),
            "date": date, "city": city.strip(), "isUDL": is_udl,
            "chuCategory": chu_category,
            "links": {
                "men": link_obj(link_men),
                "menAvg": link_obj(link_menavg),
                "women": link_obj(link_women),
                "womenAvg": link_obj(link_womenavg),
                "tournament": link_obj(link_tour),
            },
            "medals": None,
        })
    return tournaments


# ---------------------------------------------------------------------------
# 2) "Призери етапів кубків ВФД"
#    Рядок 1 (індекс 0): глобальні підписи колонок B..M = "1".."11","ЧУ"
#    Далі йдуть блоки по роках: рядок з роком у колонці A + містами по
#    колонках, потім 3-4 рядки призерів (1-ше, 2-ге, 3-тє... місце).
#    Внизу — підсумкова таблиця медального заліку (шапка "Гравець").
# ---------------------------------------------------------------------------
def parse_prizes(rows):
    if not rows:
        return {}, []

    header = rows[0]
    stage_labels = {}
    for idx, label in enumerate(header):
        if idx == 0:
            continue
        label = label.strip()
        if label:
            stage_labels[idx] = label  # "1".."11" or "ЧУ"

    year_data = {}
    aggregate = []
    in_aggregate = False

    i = 1
    n = len(rows)
    while i < n:
        row = rows[i]
        if not row or not any(c.strip() for c in row):
            i += 1
            continue

        col_a = row[0].strip() if len(row) > 0 else ""
        col_b = row[1].strip() if len(row) > 1 else ""

        if col_b == "Гравець":
            in_aggregate = True
            i += 1
            continue

        if in_aggregate:
            if col_a and len(row) > 6:
                try:
                    rank = int(col_a)
                except ValueError:
                    rank = None
                name = row[1].strip()

                def gi(x):
                    x = (x or "").strip()
                    return int(x) if x.isdigit() else 0

                if name:
                    aggregate.append({
                        "rank": rank, "name": name,
                        "gold": gi(row[2]), "silver": gi(row[3]), "bronze": gi(row[4]),
                        "champUA": gi(row[5]), "total": gi(row[6]),
                    })
            i += 1
            continue

        if re.match(r"^\d{4}$", col_a):
            year = int(col_a)
            year_data.setdefault(year, {})
            city_row = row

            podium_rows = []
            j = i + 1
            while j < n:
                nrow = rows[j]
                if not nrow or not any(c.strip() for c in nrow):
                    break
                ncol_a = nrow[0].strip() if len(nrow) > 0 else ""
                ncol_b = nrow[1].strip() if len(nrow) > 1 else ""
                if re.match(r"^\d{4}$", ncol_a) or ncol_b == "Гравець":
                    break
                podium_rows.append(nrow)
                j += 1

            for col_idx, stage_label in stage_labels.items():
                city = city_row[col_idx].strip() if col_idx < len(city_row) else ""
                if not city:
                    continue
                names = []
                for prow in podium_rows:
                    val = prow[col_idx].strip() if col_idx < len(prow) else ""
                    names.append(val or None)
                while names and names[-1] is None:
                    names.pop()
                if names:
                    year_data[year][stage_label] = {"city": city, "podium": names}
            i = j
            continue

        i += 1

    return year_data, aggregate


def parse_champion_category_header(text):
    """'ЧУ Команди Чоловіки' -> ('команди', 'men'); 'ЧУ мікст' -> ('мікст', None).
    Написано гнучко (регексами), щоб нові категорії, додані в майбутньому в
    таблицю, розпізнавались автоматично без зміни коду."""
    t = text.strip()
    gender = None
    low = t.lower()
    if "чоловіки" in low:
        gender = "men"
        t = re.sub(r"чоловіки", "", t, flags=re.IGNORECASE).strip()
    elif "жінки" in low:
        gender = "women"
        t = re.sub(r"жінки", "", t, flags=re.IGNORECASE).strip()
    t = re.sub(r"^чу\s*", "", t, flags=re.IGNORECASE).strip()
    category_key = t.lower().replace(" ", "")
    return category_key, gender


def parse_champions_sheet(rows, name_index, canonical_names):
    """Парсить таблицю 'Чемпіони України та призери' (окрема вкладка).
    Формат: рядок-заголовок року (колонка A = рік, B.. = назви категорій),
    далі 12 рядків — по 4 рядки на кожне з 3 місць (місце вказане в колонці A
    лише на першому з 4 рядків; команди/пари використовують кілька рядків
    для імен партнерів/гравців команди, одиночні категорії — лише перший).
    Повертає плаский список записів для незалежного відображення на сайті —
    НЕ змішується і не підсумовується з жодною іншою статистикою турнірів."""
    records = []
    i = 0
    n = len(rows)
    while i < n:
        row = rows[i]
        col_a = row[0].strip() if len(row) > 0 else ""
        col_b = row[1].strip() if len(row) > 1 else ""

        if re.match(r"^\d{4}$", col_a) and col_b:
            year = int(col_a)
            categories = {}  # col_idx -> (category_key, gender)
            for col_idx in range(1, len(row)):
                label = row[col_idx].strip() if col_idx < len(row) else ""
                if label:
                    categories[col_idx] = parse_champion_category_header(label)

            # 12 рядків даних: по 4 на кожне з місць 1, 2, 3
            block_start = i + 1
            for place_offset in range(3):
                place = place_offset + 1
                group_start = block_start + place_offset * 4
                group_rows = rows[group_start:group_start + 4]
                for col_idx, (cat_key, gender) in categories.items():
                    names = []
                    for grow in group_rows:
                        val = grow[col_idx].strip() if col_idx < len(grow) else ""
                        if val:
                            names.append(resolve_name(val, name_index, canonical_names))
                    if names:
                        records.append({
                            "year": year, "category": cat_key, "gender": gender,
                            "place": place, "names": names,
                        })
            i = block_start + 12
            continue
        i += 1

    return records


def parse_bio_sheet(rows, name_index, canonical_names):
    """Парсить таблицю 'BIO Гравців' (окрема вкладка, наповнюється самими
    гравцями). Формат: рядок 0 — груповий заголовок ('Дроти' над трьома
    колонками), рядок 1 — реальні заголовки колонок, рядок 2 — рядок
    фільтра (порожній), дані з рядка 3. Повертає {ім'я: {...біо-поля...}},
    включаючи лише НЕПОРОЖНІ поля для кожного гравця."""
    bios = {}
    for row in rows[3:]:
        if len(row) < 2 or not row[1].strip():
            continue
        raw_name = row[1].strip()
        name = resolve_name(raw_name, name_index, canonical_names)

        def cell(idx):
            return row[idx].strip() if idx < len(row) else ""

        bio = {}
        if cell(2):
            bio["nickname"] = cell(2)
        if cell(3):
            bio["birthDate"] = cell(3)
        if cell(4):
            bio["region"] = cell(4)
        if cell(5):
            bio["startYear"] = cell(5)
        if cell(6):
            bio["dartsManufacturer"] = cell(6)
        if cell(7):
            bio["dartsModel"] = cell(7)
        if cell(8):
            bio["dartsWeight"] = cell(8)

        if bio:
            bios[name] = bio

    return bios


def build_leaderboard_from_podiums(year_data):
    """Рахує медальний залік самостійно з даних подіумів (а не з таблиці,
    яку користувач вручну підбивав в Excel і де можливі помилки).
    Бронза рахується для КОЖНОГО імені в podium[2:] — тобто за 2013–2024,
    де обидва півфіналісти вважались призерами, це дає по 2 бронзи за етап."""
    from collections import defaultdict

    totals = defaultdict(lambda: {"gold": 0, "silver": 0, "bronze": 0, "champUA": 0})
    for stages in year_data.values():
        for key, entry in stages.items():
            podium = entry.get("podium", [])
            is_chu = key == "ЧУ"
            if len(podium) > 0 and podium[0]:
                totals[podium[0]]["gold"] += 1
                if is_chu:
                    totals[podium[0]]["champUA"] += 1
            if len(podium) > 1 and podium[1]:
                totals[podium[1]]["silver"] += 1
            for name in podium[2:]:
                if name:
                    totals[name]["bronze"] += 1

    rows = []
    for name, t in totals.items():
        total = t["gold"] + t["silver"] + t["bronze"]
        rows.append({
            "name": name, "gold": t["gold"], "silver": t["silver"],
            "bronze": t["bronze"], "champUA": t["champUA"], "total": total,
        })
    rows.sort(key=lambda r: (-r["gold"], -r["silver"], -r["bronze"], r["name"]))
    for i, r in enumerate(rows):
        r["rank"] = i + 1
    return rows


# У таблиці "Призери" міста часто скорочені інакше, ніж у таблиці турнірів
# ("КР" замість "Кривий Ріг" тощо) — без цього зіставлення по місту не
# спрацьовує, і турнір лишається без призерів, хоча дані насправді є.
CITY_ALIASES = {
    "КР": "Кривий Ріг",
    "Волинь": "Луцьк",
    "Київ опен": "Київ",
    "Kyiv Masters": "Київ",
    "Київ Мастерз": "Київ",
    "Київ мастерз": "Київ",
    "UA Open": "Одеса",
}


def normalize_city(city):
    return CITY_ALIASES.get(city, city)


def extract_stage(fmt):
    m = re.search(r"(\d+)\s*етап", fmt)
    if m:
        return m.group(1)
    if "Фінал" in fmt:
        return "FINAL"
    return None


# ---------------------------------------------------------------------------
# "Кубок України" — рейтинги за сезон (15+ вкладок, одна на рік+стать).
# Структура колонок різна з року в рік (інколи є "Місто"/"Регіон", інколи
# немає; кількість етапів різна) — тому визначаємо колонки-з-очками
# автоматично: якщо більшість заповнених клітинок колонки — числа, це
# етап/рейтинг, інакше — описова колонка (Місто, Регіон), яку пропускаємо.
# ---------------------------------------------------------------------------
NAME_SUFFIX_RE = re.compile(r"\s+[+\-=]\d*\s*$")


def clean_player_name(raw):
    """Деякі роки мають доклеєний до імені індикатор зміни місця
    ('Залевський Володимир =', 'Мелашенко Владислав +2') — прибираємо його."""
    return NAME_SUFFIX_RE.sub("", raw.strip()).strip()


def is_mostly_numeric(values):
    # "-" означає "не брав участі в цьому етапі" — це так само "порожньо",
    # як і справжня порожня клітинка, а не текстове значення.
    non_empty = [v for v in values if v.strip() and v.strip() != "-"]
    if not non_empty:
        return False
    numeric = sum(1 for v in non_empty if parse_num(v) is not None)
    return numeric / len(non_empty) >= 0.6


def parse_ratings_sheet(rows):
    """Розбирає одну вкладку рейтингу. Повертає {"columns": [...], "rows": [...]}
    або None, якщо структура не розпізнана (наприклад, порожня вкладка)."""
    header_idx = None
    for i, row in enumerate(rows):
        if len(row) > 1 and row[1].strip() in ("Гравець", "Name"):
            header_idx = i
            break
    if header_idx is None:
        return None

    header = rows[header_idx]
    data_rows = rows[header_idx + 1:]

    ncols = len(header)
    score_cols = []
    total_col = None
    for c in range(2, ncols):
        col_values = [r[c] if c < len(r) else "" for r in data_rows]
        if not is_mostly_numeric(col_values):
            continue
        label = header[c].split("\n")[0].strip() if header[c].strip() else f"Колонка {c}"
        is_total = bool(re.search(r"рейтинг|сума", header[c], re.IGNORECASE))
        if is_total and total_col is None:
            total_col = c
        else:
            score_cols.append((c, label))

    if total_col is None and score_cols:
        # немає явного підпису "рейтинг"/"сума" — беремо останню числову колонку
        total_col = score_cols[-1][0]
        score_cols = score_cols[:-1]

    if total_col is None:
        return None

    out_rows = []
    for r in data_rows:
        if len(r) < 2 or not r[1].strip():
            continue
        rank_raw = r[0].strip() if len(r) > 0 else ""
        name_raw = r[1].strip()
        if not name_raw:
            continue
        name = clean_player_name(name_raw)
        scores = [parse_num(r[c] if c < len(r) else "") for c, _ in score_cols]
        total = parse_num(r[total_col] if total_col < len(r) else "")
        out_rows.append({"rank": rank_raw, "name": name, "scores": scores, "total": total})

    # сортуємо за рейтингом на випадок, якщо вихідні рядки йшли не по порядку
    out_rows.sort(key=lambda x: -(x["total"] or 0))

    # Перераховуємо саме число місця за фінальною позицією (1,2,3,...), а не
    # довіряємо "сирому" значенню з таблиці — інакше, якщо в джерелі для
    # лідера стояло щось інше за "1" (порожньо, значок тощо), після сортування
    # за сумою перше місце просто "губилось" би з таблиці на сайті.
    for i, row in enumerate(out_rows, start=1):
        row["rank"] = str(i)

    return {"columns": [label for _, label in score_cols], "rows": out_rows}


def build_ratings(sources_path, name_index=None, canonical_names=None):
    try:
        with open(sources_path, encoding="utf-8") as f:
            config = json.load(f)
    except FileNotFoundError:
        print(f"  WARNING: {sources_path} not found, skipping ratings")
        return {}

    ratings = {}
    for sheet in config["sheets"]:
        gid, label = sheet["gid"], sheet["label"]
        year_match = re.search(r"\d{4}", label)
        year = year_match.group(0) if year_match else "0000"
        gender = "women" if "Жін" in label else "men"
        key = f"{gender}_{year}"

        csv_url = f"https://docs.google.com/spreadsheets/d/{config['spreadsheetId']}/export?format=csv&gid={gid}"
        try:
            rows = fetch_csv(csv_url)
            parsed = parse_ratings_sheet(rows)
            if parsed:
                if name_index is not None:
                    for row in parsed["rows"]:
                        row["name"] = resolve_name(row["name"], name_index, canonical_names)
                parsed["label"] = label
                parsed["year"] = year
                parsed["gender"] = gender
                ratings[key] = parsed
                print(f"  {label}: {len(parsed['rows'])} players, {len(parsed['columns'])} stages")
            else:
                print(f"  {label}: could not parse (unrecognised structure), skipping")
        except Exception as e:
            print(f"  {label}: fetch failed ({e}), skipping")
    return ratings


# ---------------------------------------------------------------------------
# Nakka (n01darts.com) — реальна статистика й призери напряму з офіційного
# публічного API (без авторизації, без оплати — тільки читання):
# https://push.n01darts.com/api/v1/n01_api_manual_en.html
# ---------------------------------------------------------------------------
NAKKA_TDID_RE = re.compile(r"[?&]id=([a-zA-Z0-9_]+)")


def extract_tdid(url):
    """Дістає tdid (наприклад 't_NtXd_3172') з посилання на n01darts.com."""
    if not url or "n01darts.com" not in url:
        return None
    m = NAKKA_TDID_RE.search(url)
    return m.group(1) if m else None


def fetch_json(url):
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (vfd-darts-sync)"})
    with urllib.request.urlopen(req, timeout=20) as resp:
        return json.loads(resp.read().decode("utf-8"))


def player_avg(stat):
    darts = stat.get("darts") or 0
    score = stat.get("score") or 0
    if darts <= 0:
        return None
    return round(score / darts * 3, 2)


def fetch_nakka_tournament(tdid, cache):
    """Тягне список учасників (tpid->ім'я) і статистику для одного tdid.
    Кешується, бо той самий tdid іноді трапляється в кількох колонках."""
    if tdid in cache:
        return cache[tdid]

    result = None
    try:
        get_resp = fetch_json(f"{NAKKA_API_BASE}/tournament/get?tdid={tdid}&entry=1")
        time.sleep(0.05)
        stats_resp = fetch_json(f"{NAKKA_API_BASE}/tournament/stats?tdid={tdid}&kind=stats_list")
        time.sleep(0.05)

        if get_resp.get("result") == 0 and stats_resp.get("result") == 0:
            entries = {
                e["tpid"]: e["name"]
                for e in get_resp.get("tournament", {}).get("entry_list", [])
                if "tpid" in e and "name" in e
            }
            result = {"entries": entries, "stats": stats_resp.get("stats", {})}
    except Exception as e:
        print(f"    Nakka API fetch failed for {tdid}: {e}")

    cache[tdid] = result
    return result


def build_name_index(*name_lists):
    """Будує словник 'Прізвище' -> 'Прізвище Ім'я', але ТІЛЬКИ для прізвищ,
    де в нашій базі є РІВНО ОДНА людина. Якщо прізвище носять кілька різних
    гравців (напр. і "Гринів Олександр", і "Гринів Юрій") — беремо його
    заднім числом, оскільки автоматично вгадати, кого мали на увазі,
    неможливо, і це раніше призводило до помилкового злиття різних людей."""
    by_surname = {}
    for names in name_lists:
        for name in names:
            if not name:
                continue
            parts = name.strip().split()
            if len(parts) < 2:
                continue
            surname = parts[0]
            by_surname.setdefault(surname, set()).add(name.strip())
    return {surname: next(iter(variants)) for surname, variants in by_surname.items() if len(variants) == 1}


def build_canonical_names(*name_lists):
    """Повний список канонічних імен (для нечіткого зіставлення ЦІЛОГО
    імені) — беремо з найнадійнішого джерела: самостійно порахований
    медальний залік (він завжди українською, завжди "Прізвище Ім'я")."""
    from collections import Counter
    counter = Counter()
    for names in name_lists:
        for name in names:
            if name and len(name.strip().split()) >= 2:
                counter[name.strip()] += 1
    # найчастіші написання йдуть першими — при однаковій схожості обирається популярніший варіант
    return [name for name, _ in counter.most_common()]


# Пари імен, які насправді позначають ОДНУ Й ТУ Ж людину (підтверджено
# вручну) — прізвище там настільки різне, що жоден автоматичний алгоритм
# зіставлення не може (і не повинен) вгадати це сам. Ключ -> буде замінено
# на значення.
# Єдине, вручну перевірене джерело правди для глобального пошуку гравця на
# сайті ("Знайти гравця"). НЕ збирається динамічно з живих даних (Nakka,
# рейтинги тощо) — саме динамічний збір і спричиняв постійні дублікати та
# витік пар/команд у пошук. Це статичний список, вручну звірений і виправлений
# у Excel-файлі; оновлюється лише коли власник сайту надсилає новий
# перевірений список.
CURATED_PLAYER_NAMES = [
    "Абчугський Олександр",
    "Аванесов Артур",
    "Аверін Сергій",
    "Агневщиков Олег",
    "Агневщиков Юрій",
    "Агневщіков Анатолій",
    "Адамко Тарас",
    "Акользін Дмитро",
    "Алексеєнко Олександр",
    "Алхімов Андрій",
    "Ананенко Альона",
    "Ананенко Сергій",
    "Андоськін Олександр",
    "Андрощук Олександр",
    "Андрєєв Антон",
    "Антіпов Денис",
    "Антіпов Іван",
    "Анісімов Денис",
    "Анісімов Іван",
    "Анісімова Світлана",
    "Аніщик Денис",
    "Арійчук Наталія",
    "Арійчук Олександр",
    "Арійчук Святослав",
    "Аспербаккен Кай",
    "Афонін Генадій",
    "Ахтирский Євген",
    "Ашколуненко Михайло",
    "Бабанін Дмитро",
    "Багрянцев Іван",
    "Бадира Олександр",
    "Байдоха Анатолій",
    "Балаж Людвик",
    "Баранов Олександр",
    "Барсуков Віталій",
    "Баширов Ярослав",
    "Баштаннік Дмитро",
    "Бейреш Ласло",
    "Белянін Андрій",
    "Беляєв Сергій",
    "Бендрик Олег",
    "Березін Руслан",
    "Береш Карол",
    "Берніков Олексій",
    "Бершак Єлизавета",
    "Бистров Володимир",
    "Богдан Віталій",
    "Богданова Діна",
    "Бойчук Людмила",
    "Бондаренко Валерій",
    "Бондарєв Євген",
    "Борисенко Сергій",
    "Борисова Марина",
    "Боровик Олександр",
    "Бородулін Дмитро",
    "Бородуліна Ганна",
    "Боцюн Костянтин",
    "Боярчук Ірина",
    "Боєв Віктор",
    "Братченко Вадим",
    "Брежнєв Олександр",
    "Брежнєв Сергій",
    "Бреус Неля",
    "Бублик В'ячеслав",
    "Бублик Лідія",
    "Бугаєнко Олександр",
    "Будик Борис",
    "Будик Зоя",
    "Букетова Iрина",
    "Бульбах Вадим",
    "Буравльов Костянтин",
    "Бургелов Олег",
    "Бустебакке Роар",
    "Бутов Олександр",
    "Бутрій Володимир",
    "Буханцов Тарас",
    "Бушуй Олексій",
    "Бушуй Сергій",
    "Бєлов Артем",
    "Білаш Вікторія",
    "Білодід Христина",
    "Білоус Данило",
    "Білоус Жанна",
    "Білоус Іван",
    "Бінат Ігор",
    "Валуце Олексій",
    "Ванда Євген",
    "Варварук  Євген",
    "Варивода Володимир",
    "Варивода Сергій",
    "Василевський Володимир",
    "Васильков Владислав",
    "Васюра Андрій",
    "Ватт Денніс",
    "Вахнюк Дмитро",
    "Вацлавскі Лукаш",
    "Ващенко Євген",
    "Ващук Павло",
    "Ведмідь Андрій",
    "Веремєєв Богдан",
    "Верхоляк Богдан",
    "Верхоляк Любомир",
    "Верхоляк Роман",
    "Виговський Антон",
    "Вильданов Игор",
    "Висковатова Ганна",
    "Витковський Анатолій",
    "Вишневецький Олег",
    "Вишневський Олег",
    "Владика Юрій",
    "Власенко Віталій",
    "Волков Владислав",
    "Воловод Ростислав",
    "Волчук Руслан",
    "Вонхольський Олександр",
    "Воронюк Михайло",
    "Воронін Олександр",
    "Вінничук Микола",
    "Вінцюк Тарас",
    "Вісковатова Ганна",
    "Гавриленко Владислав",
    "Гавриленко Роман",
    "Гавриш Ольга",
    "Гайдукова Наталья",
    "Гайсан Ангеліна",
    "Гайсан Марʼяна",
    "Гакман Степан",
    "Галичанський Артур",
    "Галичанський Тімур",
    "Галуба Ярек",
    "Ганечко Арина",
    "Ганечко Марина",
    "Гарапко Інна",
    "Гарасимчук Игор",
    "Генерал Андрій",
    "Герасименко Юлія",
    "Герман Олег",
    "Гетманцев Олександр",
    "Глаголева Тетяна",
    "Гладка Ірина",
    "Глущенко Віталій",
    "Гниденко Валентин",
    "Гнилосир Сергій",
    "Гой Оксана",
    "Гойколов Олександр",
    "Головень Юрій",
    "Гончаренко Мар'яна",
    "Гончаренко Олександр",
    "Гончаренко Інна",
    "Гончаров Едуард",
    "Гончарова Марина",
    "Гордієнко Христина",
    "Горняк Іштван",
    "Городецький Ілля",
    "Горяйнова Марія",
    "Гоян Дмитро",
    "Градовий Сергій",
    "Гриб Олександр",
    "Григоренко Андрій",
    "Григоренко Віктор",
    "Григоренко Денис",
    "Григоренко Костянтин",
    "Гринько Віталій",
    "Гринів Олександр",
    "Гринів Юрій",
    "Грицай Іван",
    "Гросов Олексій",
    "Грушовець Ольга",
    "Грєков Юрій",
    "Губа Дмитро",
    "Губіна Наталія",
    "Гудзь Ліна",
    "Гулякевич Олексей",
    "Гурський Борис",
    "Гутнік Анатолій",
    "Данилюк Олександр",
    "Дегтярьов Михайло",
    "Денисьєв Олександр",
    "Денисьєв Ілля",
    "Денов Микола",
    "Деркач Дмитро",
    "Десинський Максим",
    "Дещенко Юрій",
    "Дзебань Дмитро",
    "Дзюбій Юлія",
    "Дмитров Дмитро",
    "Добровольський Жолудь",
    "Доган Ісмаіл",
    "Долганенко Денис",
    "Доманін Євген",
    "Дорошенко Ігор",
    "Дроздов Євген",
    "Дрєєв Олександр",
    "Діденко Оксана",
    "Дідов Василь",
    "Еріксон Хенінг",
    "Жадан Олександр",
    "Жигура Віталій",
    "Жук Валерій",
    "Жуковський Микола",
    "Журавель Алла",
    "Заборний Андрій",
    "Заволокін Дмитро",
    "Зайцева Інна",
    "Закіров Марат",
    "Залевський Володимир",
    "Зарайський Володимир",
    "Захарова Зоя",
    "Зварич Віктор",
    "Зволякевич Олена",
    "Звягін Юрій",
    "Звягіна Анастасія",
    "Звєздін Євген",
    "Зеленько Віталій",
    "Зелюкіна Світлана",
    "Зелінська Катерина",
    "Зеркаль Сергій",
    "Зиков Андрій",
    "Зольдін Фелікс",
    "Зольдіна Євгенія",
    "Зотов Іван",
    "Зотова Анна",
    "Зубаль Василь",
    "Зубков Юрій",
    "Зуєв Олег",
    "Зязюля Андрій",
    "Кабанова Світлана",
    "Казинський Олег",
    "Калаур Інна",
    "Каліберда Володимир",
    "Калінін Роман",
    "Калініченко Едуард",
    "Калініченко Сергій",
    "Камельков Євген",
    "Камелькова Валерія",
    "Камелькова Людмила",
    "Каменицки Тибор",
    "Кандяк Павло",
    "Каньшин Олександр",
    "Капран Анастасія",
    "Капран Олександр",
    "Кара Євген",
    "Карабан Юрій",
    "Караман Михайло",
    "Карнаух Валентина",
    "Каропкін Євген",
    "Карпевич Олександр",
    "Карпенко Юрій",
    "Касапчук Анастасія",
    "Касьянов Вадим",
    "Катишев Олександр",
    "Качура Катерина",
    "Квасков Павло",
    "Квиклене Рената",
    "Келіберда Володимир",
    "Кенес Василій",
    "Кива Дмитро",
    "Кива Ірина",
    "Кисельов Дмитро",
    "Клочек Ксенія",
    "Клочко Володимир",
    "Клочко Марина",
    "Клюйков Роман",
    "Князь Каріна",
    "Кобзар Ігор",
    "Коваленко Віктор",
    "Коваль Анастасія",
    "Коваль Олександр",
    "Ковальов Андрій",
    "Ковальов Дмитро",
    "Ковач Золтан",
    "Козирєв Олександр",
    "Козловський Богдан",
    "Козлюк Дмитро",
    "Колесніков Юрій",
    "Колодяжний Костянтин",
    "Коломієць Андрій",
    "Коломієць Ганна",
    "Коломієць Олена",
    "Кондарюк Микола",
    "Коновалов Олег",
    "Конопченко Віктор",
    "Конопченко Данило",
    "Корб'як Ігор",
    "Кордівський Арсен",
    "Король Василь",
    "Корольова Еліна",
    "Коропкін Євген",
    "Коростельов Дмитро",
    "Корпусов Андрій",
    "Корчак Вікторія",
    "Корчак Сергій",
    "Косигіна Ольга",
    "Косолапов Олег",
    "Костильов Євгеній",
    "Костів Валерія",
    "Костів Оксана",
    "Котенко Олег",
    "Котляр Юрій",
    "Котнюк Богдан",
    "Котузяк Олег",
    "Котулек Еркут",
    "Крамський Кирило",
    "Крамський Сергій",
    "Краплін Игор",
    "Кременецький Іван",
    "Крестніков Юрій",
    "Криницький Юрій",
    "Крутій Володимир",
    "Кубчинський Микола",
    "Кудрявцев Руслан",
    "Кудрявцев Євген",
    "Кудінов Павло",
    "Кузнєцов Павло",
    "Кузнєцова Вікторія",
    "Кузовкін Сергій",
    "Кузьменко Роман",
    "Кулаков Володимир",
    "Кульбаченко Олег",
    "Кулік Віталій",
    "Кулінчик Дмитро",
    "Кумовицький Павло",
    "Кунченко Євген",
    "Кураш Богдан",
    "Кургузов Олег",
    "Курочка Євген",
    "Куча Валерій",
    "Куча Леонід",
    "Кучеренко Наталя",
    "Кучмій Андрій",
    "Кідиба Антон",
    "Кільчицький Олександр",
    "Кіфор Ярослав",
    "Лабик Ілля",
    "Лавренюк Сергій",
    "Лавринчук Катерина",
    "Лагутін Андрій",
    "Ланда Володимир",
    "Ландин Ілля",
    "Лановенко Ксенія",
    "Ларіна Тетяна",
    "Ледєньов Вадим",
    "Ледєньова Наталія",
    "Лемещук Олександр",
    "Леонова Єлізавета",
    "Лепень Олександр",
    "Лец Дмитро",
    "Лещенко Юрій",
    "Липчевский Артур",
    "Лисаков Олексій",
    "Лобода Валерій",
    "Логоша Євген",
    "Лопиш Норбер",
    "Лопухова Ганна",
    "Лосєва Марія",
    "Луговська Людмила",
    "Лужанков Дмитро",
    "Лузан Юлія",
    "Лук'янов Євген",
    "Луков Олександр",
    "Лукова Вікторія",
    "Львов Роман",
    "Льохарт Олег",
    "Льянов Олег",
    "Любих Сергій",
    "Лясота Наталія",
    "Лях Свiтлана",
    "Лізенер Леон",
    "Мажан Костянтин",
    "Макагон Сергій",
    "Макаєв Віктор",
    "Малюхович Євген",
    "Мамика Олександр",
    "Марет Лійрі",
    "Марига Руслан",
    "Марков Сергій",
    "Мартов Володимир",
    "Марфола Оксана",
    "Марченко Денис",
    "Масловський Михайло",
    "Маслянок Вікторія",
    "Маслій Євген",
    "Матвиєнков Дмитро",
    "Матвіїшин Михайло",
    "Матейчук Юрій",
    "Матус Олег",
    "Матюшенко Володимир",
    "Матійцов Ростислав",
    "Махневич Валерій",
    "Мегера Олександра",
    "Медведев Олександр",
    "Медведєв Максим",
    "Медведєв Олександр",
    "Медведєв Єгор",
    "Медвєдська Людмила",
    "Мелах Костянтин",
    "Мелашенко Владислав",
    "Мельник Вікторія",
    "Мельниченко Оксана",
    "Мельтєва Ірина",
    "Мигалюк Родіон",
    "Микитин Ярослав",
    "Миколюк Яна",
    "Мильо Тарас",
    "Миндра Євген",
    "Митчик Тетяна",
    "Могилевський Руслан",
    "Мойсеєнков Дмитро",
    "Мойсюк Віталій",
    "Мокан Денис",
    "Мокан Микола",
    "Молдован Вероніка",
    "Молдован Емілія",
    "Молдован Сергій",
    "Молодецкий Ростислав",
    "Молчанов Михайло",
    "Момот Сергій",
    "Мороз Артем",
    "Мороз Владислав",
    "Мороз Сергій",
    "Мороз Ігор",
    "Морозов Андрій",
    "Морозов Сергій",
    "Мороко Максим",
    "Мотуз Едуард",
    "Мотуз Єгор",
    "Моісеєнко Віталіна",
    "Моісюк Віталій",
    "Мустафаєв Ільмій",
    "Мутило Максим",
    "Мігалюк Родіон",
    "Мінченок Юрій",
    "Набільський Сергій",
    "Навойчик Віталій",
    "Навроцький Дмитро",
    "Надводнюк Ганна",
    "Назаренко Владислав",
    "Назаренко Олександр",
    "Назаренко Олексій",
    "Назаренко Ольга",
    "Науменко Віталій",
    "Невінчаний Едуард",
    "Некрістов Станіслав",
    "Некрістова Наталія",
    "Нестеренко Олена",
    "Нечипуренко Артем",
    "Ниценко Ольга",
    "Новак Андрій",
    "Новак Едуард",
    "Новосад Богдан",
    "Новікова Зоя",
    "Носовський Дмитро",
    "Носівець Сергій",
    "Нужнова Тетяна",
    "Нємцов Віталій",
    "Ніколаєнко Євген",
    "Ніколенко Ігор",
    "Нікітіч Сергій",
    "Ніщик Володимир",
    "Ніщик Юліана",
    "Овелько Антон",
    "Овсієнко Євген",
    "Овчаренко Антон",
    "Овчаренко Сергій",
    "Огурцов Дмитро",
    "Озджан Мустафа",
    "Окуліцький Євген",
    "Олег Волощук",
    "Олексенко Олександр",
    "Олійник Надія",
    "Омельченко Владислав",
    "Омельченко Володимир",
    "Омелянченко Олександр",
    "Омесь Ілля",
    "Онуфраш Іван",
    "Оніщенко Тетяна",
    "Опоченко Олег",
    "Осадча Юлія",
    "Острадчук Вадим",
    "Остромецький Влад",
    "Оттерлей Ендре",
    "Павзюк Олег",
    "Павленко Сергій",
    "Павлов Олександр",
    "Павлюк Анастасія",
    "Павлюченко Марина",
    "Пайзак Синтія",
    "Пайзак Томаш",
    "Паламар Оксана",
    "Палахіна Альона",
    "Пальонний Ігор",
    "Панасюк Лілія",
    "Панасюк Сергій",
    "Панченко Олексій",
    "Пасичніченко Артрем",
    "Паскару Габріель",
    "Пекарук Олексій",
    "Пекарук Ілля",
    "Пенчук Василь",
    "Перес Володимир",
    "Перетятько Тетяна",
    "Пермякова Ольга",
    "Петрик Наталія",
    "Петрик Олександр",
    "Петухов Валерій",
    "Пивошенко Максим",
    "Пивошенко Олена",
    "Пирсіков Олександр",
    "Писаревський Вiталiй",
    "Плакса Андрій",
    "Плакса Володимир",
    "Пламада Денис",
    "Плуталов Андрій",
    "Поденко Олена",
    "Подкопаєв Андрій",
    "Подольський Святослав",
    "Подольчук Антон",
    "Покладок Олег",
    "Полтавець Світлана",
    "Полторацький Олександр",
    "Полторацький Сергій",
    "Полунін Максим",
    "Поляков-Лагода Артур",
    "Пометун Руслан",
    "Пономаренко Денис",
    "Понтус Андрій",
    "Попков Андрій",
    "Попов Юрій",
    "Попова Наталія",
    "Попович Андрій",
    "Попович Карл",
    "Потан Лілія",
    "Почапський Василь",
    "Почепко Сергій",
    "Почкай Андрій",
    "Почкай Маріанна",
    "Приходько Сергій",
    "Приходько Євген",
    "Пєлєнов Сергій",
    "Пістружак Дарія",
    "Пішта Людмила",
    "Пішта Іван",
    "Радіонов Олександр",
    "Радіонова Дарʼя",
    "Разінков Олександр",
    "Рашоянц Ігор",
    "Редзюк Евгений",
    "Редзюк Тетяна",
    "Решат Нурі Аслан",
    "Рижиков Сергій",
    "Рихлюк Олександр",
    "Ровенчук Влада",
    "Ровенчук Володимир",
    "Рогаля Марк",
    "Рогов Віталій",
    "Роговий Ігор",
    "Роземберський Іосіф",
    "Розловський Андрій",
    "Романенко Кирило",
    "Романов Олександр",
    "Ромашко Віталій",
    "Руденко Анна",
    "Рукавишников Микола",
    "Русанов Андрій",
    "Рутенберг Дмитро",
    "Сабунджуоглу Алі Енвер",
    "Савенков Антон",
    "Савчук Володимир",
    "Сагайдачный Роман",
    "Садохін Олег",
    "Сальніков Павло",
    "Самодій Петро",
    "Сапронов Петро",
    "Сарапін Олександр",
    "Сарія Олександр",
    "Сас Олег",
    "Сафаров Олександр",
    "Сахаров Сергій",
    "Саїнчук Тетяна",
    "Свирид Марія",
    "Свіда Олексій",
    "Свіда Юрій",
    "Севостьянов Юрій",
    "Секелі Пал",
    "Селєзньов Олександр",
    "Селіхова Тетяна",
    "Семейко Віталій",
    "Сень Микола",
    "Сергеєв Андрій",
    "Сергєєв Кирило",
    "Сергєєв Олексій",
    "Сидельникова Ганна",
    "Сидоренко Дмитро",
    "Сидоренко Олександр",
    "Силюк Володимир",
    "Сиром'ятников Олександр",
    "Скакун Ірина",
    "Скарлат Дмитро",
    "Скарлат Олександр",
    "Скиданчук Віктор",
    "Скрипай Дмитро",
    "Скуртов Дмитро",
    "Скуртова Анастасія",
    "Смик Володимир",
    "Смищенко Артем",
    "Смірнова Світлана",
    "Снурніков Артем",
    "Соколов Володимир",
    "Соколов Роман",
    "Соколова Вікторія",
    "Соловйова Наталія",
    "Соловьова Юлія",
    "Солтис Андрій",
    "Солтис Ганна",
    "Солтис Микита",
    "Сорокотяга Олег",
    "Софлюк Юлія",
    "Спекторенко Катерина",
    "Старіков Володимир",
    "Сташевський Валентин",
    "Стебелько Ігор",
    "Стейер Себастьян",
    "Степаненко Віталій",
    "Степанець Віталіна",
    "Степанець Ольга",
    "Стецишин Віталій",
    "Стецюк Володимир",
    "Стецюра Олена",
    "Стомаченко Ксенія",
    "Сторожук Дмитро",
    "Стребиж Татяна",
    "Стребиж Тетяна",
    "Субаши Алпер",
    "Сулім Роман",
    "Суслова Євгенія",
    "Сєлєзньов Олександр",
    "Сіташ Габор",
    "Тараненко Лілія",
    "Тарасенко Андрій",
    "Тарасюк Юрій",
    "Теличко Антон",
    "Телятник Максим",
    "Терещенко Любов",
    "Терновий Олександр",
    "Тесленко Кирило",
    "Тимонова Діана",
    "Тимченко Михайло",
    "Титков Олег",
    "Тиханов Юрій",
    "Тихобаєва Вікторія",
    "Тихоненко Віктор",
    "Тихонченко Віталій",
    "Ткаченко Сергій",
    "Ткачук Сергій",
    "Товт Золтан",
    "Томків Ірина",
    "Томчук Олег",
    "Топко Андрій",
    "Топко Олександр",
    "Топчу Владислав",
    "Топчук Анатолій",
    "Торський Тарас",
    "Торський Євген",
    "Точка Петро",
    "Трофімов Дмитро",
    "Троценко Артем",
    "Трубецькой Петро",
    "Тужикова Світлана",
    "Тямолов Роман",
    "Тямолов Ростислав",
    "Тіщенко Богдан",
    "Удовиченко Едуард",
    "Улезко Анатолій",
    "Усетс Терьєр",
    "Усик Артем",
    "Усик Сергій",
    "Утембаєв Андрій",
    "Фатех Дін",
    "Федоришина Алла",
    "Федотова Ірина",
    "Федуна Віктор",
    "Федуна Ігор",
    "Ферар Крістіна",
    "Ферар Максим",
    "Фехтел Анатолій",
    "Фирсиков Виктор",
    "Форманюк Андрій",
    "Франчук Андрій",
    "Філіпець Дмитро",
    "Фісяк Павло",
    "Халеєв Костянтин",
    "Харченко Тетяна",
    "Ходяков Володимир",
    "Ходєєв Микола",
    "Холліуелл Вейн",
    "Хоменко Андрій",
    "Хоменко Владислав",
    "Хоменко Володимир",
    "Хоміч Володимир",
    "Худяшов Станіслав",
    "Хуренко Сергій",
    "Хусаінов Сергій",
    "Хімко Олександр",
    "Хімчак Сергій",
    "Царолунга Дмитро",
    "Цебратенко Андрій",
    "Цибух Антон",
    "Циганков Павло",
    "Цитович Андрій",
    "Цілуйко Владислав",
    "Чабанова Анна",
    "Чайковський Руслан",
    "Чебаненко Олексій",
    "Чекановський Арсеній",
    "Чепак Василь",
    "Чепак Сергій",
    "Чепель Ігор",
    "Чепурний Едуард",
    "Черватюк Ірина",
    "Червинська Ніна",
    "Червинська Ніні",
    "Черевко Свiтлана",
    "Чередниченко Сергій",
    "Черкас Павло",
    "Черкасов Костянтин",
    "Черноброва Олександра",
    "Черченко Михайло",
    "Чижмадія Томаш",
    "Чик Едуард",
    "Чорба Золтан",
    "Чорний Віталій",
    "Чорний Іван",
    "Чорняк Михай",
    "Чубат Андрій",
    "Чумак Ганна",
    "Чумак Дмитро",
    "Чіголя Артур",
    "Чікало Костянтин",
    "Шаблій Ігор",
    "Шапка Євген",
    "Швець Андрій",
    "Шевченко Сергій",
    "Шевчик Анастасія",
    "Шевчук Дмитро",
    "Шемет Наталія",
    "Шепута Володимир",
    "Шийка Артур",
    "Шикирюк Євген",
    "Шильцин Юрій",
    "Шинкаренко Василь",
    "Шипулін Олексей",
    "Шипулін Сергій",
    "Шипіцин Олег",
    "Ширяєв Дмитро",
    "Шклярук Наталія",
    "Шклярук Юлія",
    "Шкуров Артем",
    "Шляхов Андрій",
    "Шляхов Сергій",
    "Шмідтке Анджей",
    "Шпак Максим",
    "Шпигун Володимир",
    "Шпитко Вікторія",
    "Штанько Василь",
    "Штанько Катерина",
    "Штувбейна Альона",
    "Штурко Дмитро",
    "Шум Олександр",
    "Шумко Олександр",
    "Шумілов Дмитро",
    "Шумілова Ірина",
    "Шуплєцов Станислав",
    "Шуплєцова Зоя",
    "Щолок Володимир",
    "Юджель Сінан",
    "Юкнаїт Альгіна",
    "Юрченко Володимир",
    "Юша Олексій",
    "Ющик Сергій",
    "Яворський Костянтин",
    "Язган Котулек Меліке",
    "Якименко Віталій",
    "Якименко Дмитро",
    "Якименко Катерина",
    "Якімов Павло",
    "Янгу Панайотіс",
    "Яненко Сергій",
    "Ясинський Владислав",
    "Яскевич Олена",
    "Яцюра Тетяна",
    "Євмєнєнков Максим",
    "Ємельянов Олександр",
    "Єрашов Віталій",
    "Єрашов Дмитро",
    "Єрмаков Сергій",
    "Єрол Умут",
    "Єрчик Маргарита",
    "Єрєменко Василій",
    "Єськов Олег",
    "Іванов Олександр",
    "Іванов Сергій",
    "Іванщишин Орест",
    "Івасюк Віталій",
    "Іващенко Ірина",
    "Івлєв Олександр",
    "Ігнатюк Наталія",
    "Ізосімов Анатолій",
    "Ільясова Альбіна",
    "Іргізова Нелля",
    "Іфтоде Мірела",
    "Ішутін Ігор",
    "Іщенко Сергій",
]


def is_pair_or_team_name(name):
    """Той самий евристичний фільтр, що й на фронтенді (isIndividualPlayerName):
    пари/дуети (через "/" чи "\\") та назви команд (ВЕЛИКИМИ ЛІТЕРАМИ) — це
    НЕ окремі гравці, тому їх не можна автоматично додавати до списку пошуку."""
    if not name:
        return True
    if "/" in name or "\\" in name or "," in name:
        return True
    if len(name) > 3 and name.upper() == name and any(c.isalpha() for c in name):
        return True
    return False


MANUAL_NAME_ALIASES = {
    "Sol Deen": "Фатех Дін",
    "Мєльтєва Ірина": "Мельтєва Ірина",
    "Букетова Ірина": "Букетова Iрина",
    "Зарайский Володимир": "Зарайський Володимир",
    "Кулик Віталій": "Кулік Віталій",
    "Онищенко Тетяна": "Оніщенко Тетяна",
    "Сєргєєв Андрій": "Сергеєв Андрій",
    "Ізосімом Анатолій": "Ізосімов Анатолій",
    # --- Виправлення з третього раунду перевірки Excel (уся база, включно з архівом рейтингів) ---
    "Ємельянов Олесандр": "Ємельянов Олександр",
    "Ігнатюк Наталя": "Ігнатюк Наталія",
    "Іргізова Неля": "Іргізова Нелля",
    "Агневщиков Анатолий": "Агневщіков Анатолій",
    "Алексеенко Александр": "Алексеєнко Олександр",
    "Алхимов Андрей": "Алхімов Андрій",
    "Андоськин Александр": "Андоськін Олександр",
    "Андрощук Александр": "Андрощук Олександр",
    "Анисимов Денис": "Анісімов Денис",
    "Анисимов Иван": "Анісімов Іван",
    "Анисимова Светлана": "Анісімова Світлана",
    "Антипов Денис": "Антіпов Денис",
    "Арийчук Александр": "Арійчук Олександр",
    "Арійчук Наталя": "Арійчук Наталія",
    "Афонин Геннадий": "Афонін Генадій",
    "Афонин Геннадий А.": "Афонін Генадій",
    "Афонин Геннадий Г.": "Афонін Генадій",
    "Ахтырский Евгений": "Ахтирский Євген",
    "Ашкауленко Михаил": "Ашколуненко Михайло",
    "Багрянцев Иван": "Багрянцев Іван",
    "Бадыра Александр": "Бадира Олександр",
    "Байдоха Анатолий": "Байдоха Анатолій",
    "Баранов Александр": "Баранов Олександр",
    "Барсуков Виталий": "Барсуков Віталій",
    "Барсуков Віталий": "Барсуков Віталій",
    "Белодед Кристина": "Білодід Христина",
    "Белянин Андрей": "Белянін Андрій",
    "Березин Руслан": "Березін Руслан",
    "Богданова Дина": "Богданова Діна",
    "Боев Виктор": "Боєв Віктор",
    "Бондаренко Валерий": "Бондаренко Валерій",
    "Борисенко Сергей": "Борисенко Сергій",
    "Борисова Марія": "Борисова Марина",
    "Бородулин Дмитрий": "Бородулін Дмитро",
    "Бородулина Анна": "Бородуліна Ганна",
    "Боцюн Константин": "Боцюн Костянтин",
    "Боярчук Ирина": "Боярчук Ірина",
    "Брежнев Александр": "Брежнєв Олександр",
    "Брежнев Сергей": "Брежнєв Сергій",
    "Бублік ?": "Бублик В'ячеслав",
    "Бублік Лідія": "Бублик Лідія",
    "Буравлёв Константин": "Буравльов Костянтин",
    "Буравьов Костянтин": "Буравльов Костянтин",
    "Бурмака Павел": "Кумовицький Павло",
    "Бутов Александр": "Бутов Олександр",
    "Бутрий Владимир": "Бутрій Володимир",
    "Бушуй Алексей": "Бушуй Олексій",
    "Быстров Владимир": "Бистров Володимир",
    "Білодід Кристіна": "Білодід Христина",
    "Білодід Крістіна": "Білодід Христина",
    "Валуце Алексей": "Валуце Олексій",
    "Варварук Евгений": "Варварук  Євген",
    "Варивода": "Варивода Володимир",
    "Варивода Владимир": "Варивода Володимир",
    "Васюра Андрей": "Васюра Андрій",
    "Вахнюк Дмитрий": "Вахнюк Дмитро",
    "Веремеев Богдан": "Веремєєв Богдан",
    "Вильданов Игорь": "Вильданов Игор",
    "Витковский Анатолий": "Витковський Анатолій",
    "Власенко Виталий": "Власенко Віталій",
    "Выговский Антон": "Виговський Антон",
    "Гарапко Инна": "Гарапко Інна",
    "Гарасимчук Игорь": "Гарасимчук Игор",
    "Генерал Андрей": "Генерал Андрій",
    "Гетманцев Александр": "Гетманцев Олександр",
    "Гладка Ирина": "Гладка Ірина",
    "Гойколов Александр": "Гойколов Олександр",
    "Гончаров Эдуард": "Гончаров Едуард",
    "Гордиенко Кристина": "Гордієнко Христина",
    "Горняк Иштван": "Горняк Іштван",
    "Городецкий Илья": "Городецький Ілля",
    "Гоян Дмитрий": "Гоян Дмитро",
    "Гросов Алексей": "Гросов Олексій",
    "Губа Дмитрий": "Губа Дмитро",
    "Губина Наталья": "Губіна Наталія",
    "Гудзь Лина": "Гудзь Ліна",
    "Гулякевич Алексей": "Гулякевич Олексей",
    "Гурский Борис": "Гурський Борис",
    "Дегтярев Михаил": "Дегтярьов Михайло",
    "Денисьев Александр": "Денисьєв Олександр",
    "Денисьев Илья": "Денисьєв Ілля",
    "Деркач Дмитрий": "Деркач Дмитро",
    "Десинський": "Десинський Максим",
    "Дещенко Юрий": "Дещенко Юрій",
    "Дзебань Дмитрий": "Дзебань Дмитро",
    "Дзюбий Юлия": "Дзюбій Юлія",
    "Дзюбий Юлія": "Дзюбій Юлія",
    "Доманін Евген": "Доманін Євген",
    "Дорошенко Игорь": "Дорошенко Ігор",
    "Дроздов Евгений": "Дроздов Євген",
    "Евмененков Максим": "Євмєнєнков Максим",
    "Емельянов Александр": "Ємельянов Олександр",
    "Ерашов Виталий": "Єрашов Віталій",
    "Ерашов Дмитрий": "Єрашов Дмитро",
    "Еременко Василий": "Єрєменко Василій",
    "Ерол Умут": "Єрол Умут",
    "Еськов Олег": "Єськов Олег",
    "Жук Валерий": "Жук Валерій",
    "Заборный Андрей": "Заборний Андрій",
    "Закиров Марат": "Закіров Марат",
    "Зарайский Владимир": "Зарайський Володимир",
    "Звездин Евгений": "Звєздін Євген",
    "Зеленко Виталий": "Зеленько Віталій",
    "Зелінська": "Зелінська Катерина",
    "Зольдин Феликс": "Зольдін Фелікс",
    "Зотов Иван": "Зотов Іван",
    "Зуев": "Зуєв Олег",
    "Зыков Андрей": "Зиков Андрій",
    "Иващенко Ирина": "Іващенко Ірина",
    "Ивлев Александр": "Івлєв Олександр",
    "Ищенко Сергей": "Іщенко Сергій",
    "Калаур Инна Кривой": "Калаур Інна",
    "Камельков Евгений": "Камельков Євген",
    "Камелькова Валерия": "Камелькова Валерія",
    "Каньшин Александр": "Каньшин Олександр",
    "Караман Михаил": "Караман Михайло",
    "Карпенко Юрий": "Карпенко Юрій",
    "Катышев Александр": "Катишев Олександр",
    "Квасков Павел": "Квасков Павло",
    "Кенес Василий": "Кенес Василій",
    "Киселёв Дмитрий": "Кисельов Дмитро",
    "Клочков Владимир": "Клочко Володимир",
    "Кобзарь Игорь": "Кобзар Ігор",
    "Ковалев Дмитрий": "Ковальов Дмитро",
    "Коваль Александр": "Коваль Олександр",
    "Козлюк Дмитрий": "Козлюк Дмитро",
    "Козырев Александр": "Козирєв Олександр",
    "Колесников Юрий": "Колесніков Юрій",
    "Колодяжный Константин": "Колодяжний Костянтин",
    "Конопченко Виктор": "Конопченко Віктор",
    "Конопченко Даниил": "Конопченко Данило",
    "Королёва Элина": "Корольова Еліна",
    "Коропкин Евгений": "Коропкін Євген",
    "Коропкін Євгеній": "Коропкін Євген",
    "Костылёв Евгений": "Костильов Євгеній",
    "Косыгина Ольга": "Косигіна Ольга",
    "Крамской Сергей": "Крамський Сергій",
    "Краплин Игорь": "Краплін Игор",
    "Кресников Юрій": "Крестніков Юрій",
    "Крестников Юрий": "Крестніков Юрій",
    "Кубчинский Николай": "Кубчинський Микола",
    "Кулаков Владимир": "Кулаков Володимир",
    "Кулинчик Дмитрий": "Кулінчик Дмитро",
    "Лавринчук Екатерина": "Лавринчук Катерина",
    "Лагутін Анрій": "Лагутін Андрій",
    "Ландин Илья": "Ландин Ілля",
    "Леденёв Вадим": "Ледєньов Вадим",
    "Леденёва Наталья": "Ледєньова Наталія",
    "Ледєньова Наталя": "Ледєньова Наталія",
    "Леонова Елизавета": "Леонова Єлізавета",
    "Лец Дмитрий": "Лец Дмитро",
    "Лещенко Юрий": "Лещенко Юрій",
    "Логоша Евгений": "Логоша Євген",
    "Лопухова Анна": "Лопухова Ганна",
    "Луговская Людмила": "Луговська Людмила",
    "Лузан Юлiя": "Лузан Юлія",
    "Луков Александр": "Луков Олександр",
    "Любых Сергей": "Любих Сергій",
    "Лях": "Лях Свiтлана",
    "Мажан Константин": "Мажан Костянтин",
    "Макагон Сергей": "Макагон Сергій",
    "Макаев Виктор": "Макаєв Віктор",
    "Мамыка Александр": "Мамика Олександр",
    "Мартов Владимир": "Мартов Володимир",
    "Марыга Руслан": "Марига Руслан",
    "Маслий Евгений": "Маслій Євген",
    "Масловский Михаил": "Масловський Михайло",
    "Матвиенков Дмитрий": "Матвиєнков Дмитро",
    "Матейчук": "Матейчук Юрій",
    "Матюшенко Владимир": "Матюшенко Володимир",
    "Махневич Валерий": "Махневич Валерій",
    "Медведев Александр": "Медведев Олександр",
    "Медведская Людмила": "Медвєдська Людмила",
    "Моисеенко Виталина": "Моісеєнко Віталіна",
    "Молчанов Михаил": "Молчанов Михайло",
    "Мороз Игорь": "Мороз Ігор",
    "Мороз Сергей": "Мороз Сергій",
    "Морозов Андрей": "Морозов Андрій",
    "Морозов Сергей": "Морозов Сергій",
    "Моісеєнко": "Моісеєнко Віталіна",
    "Моісеєнко Вiталiна": "Моісеєнко Віталіна",
    "Набильский Сергей": "Набільський Сергій",
    "Навойчик Виталий": "Навойчик Віталій",
    "Навроцкий Дмитрий": "Навроцький Дмитро",
    "Назаренко Алексей": "Назаренко Олексій",
    "Невенчаний Едуард": "Невінчаний Едуард",
    "Невенчаный Эдуард": "Невінчаний Едуард",
    "Некристов Станислав": "Некрістов Станіслав",
    "Некристова Наталья": "Некрістова Наталія",
    "Некристова Наталя": "Некрістова Наталія",
    "Никитич Сергей": "Нікітіч Сергій",
    "Николаенко Евгений": "Ніколаєнко Євген",
    "Новак Эдуард": "Новак Едуард",
    "Новикова Зоя": "Новікова Зоя",
    "Носивец Сергей": "Носівець Сергій",
    "Носовский Дмитрий": "Носовський Дмитро",
    "Нямцов Віталій": "Нємцов Віталій",
    "Олексенко Александр": "Олексенко Олександр",
    "Павлов Александр": "Павлов Олександр",
    "Пайзак Синтия": "Пайзак Синтія",
    "Палённый Игорь": "Пальонний Ігор",
    "Панченко Алексей": "Панченко Олексій",
    "Пасичниченко Артём": "Пасичніченко Артрем",
    "Пасічніченко Артем": "Пасичніченко Артрем",
    "Пекарук Алексій": "Пекарук Олексій",
    "Перес Владимир": "Перес Володимир",
    "Перетятько Татьяна": "Перетятько Тетяна",
    "Петрик Наталья": "Петрик Наталія",
    "Плуталов Андрей": "Плуталов Андрій",
    "Подкопаев Андрей": "Подкопаєв Андрій",
    "Полторацкий Александр": "Полторацький Олександр",
    "Полторацкий Сергей": "Полторацький Сергій",
    "Полунин Максим": "Полунін Максим",
    "Попов Юрий": "Попов Юрій",
    "Попова Наталiя": "Попова Наталія",
    "Попович Андрей": "Попович Андрій",
    "Потан Лилия": "Потан Лілія",
    "Почапский Василий": "Почапський Василь",
    "Почепко Сергей": "Почепко Сергій",
    "Пырсиков Александр": "Пирсіков Олександр",
    "Радіонова Дар'я": "Радіонова Дарʼя",
    "Редзюк Татьяна": "Редзюк Тетяна",
    "Ровенчук Владимир": "Ровенчук Володимир",
    "Роговой Игорь": "Роговий Ігор",
    "Роземберский Иосиф": "Роземберський Іосіф",
    "Романенко Кирилл": "Романенко Кирило",
    "Ромашко Виталий": "Ромашко Віталій",
    "Рудковский Василий": "Дідов Василь",
    "Русаков Андрей": "Русанов Андрій",
    "Русанов Андрей": "Русанов Андрій",
    "Сальников Павел": "Сальніков Павло",
    "Сафаров Александр": "Сафаров Олександр",
    "Свида Юрий": "Свіда Юрій",
    "Сергеев Андрей": "Сергеєв Андрій",
    "Сидельникова Анна": "Сидельникова Ганна",
    "Сидоренко Дмитрий": "Сидоренко Дмитро",
    "Скарлат Александр": "Скарлат Олександр",
    "Скарлат Дмитрий": "Скарлат Дмитро",
    "Скуртов Дмитрий": "Скуртов Дмитро",
    "Смищенко Артём": "Смищенко Артем",
    "Снурников Артём": "Снурніков Артем",
    "Соловьёва Юлия": "Соловьова Юлія",
    "Сохаіл Соул": "Фатех Дін",
    "Спекторенко Екатерина": "Спекторенко Катерина",
    "Стариков Владимир": "Старіков Володимир",
    "Стецишин Віталий": "Стецишин Віталій",
    "Стребиж Татьяна": "Стребиж Тетяна",
    "Сулим Роман": "Сулім Роман",
    "Суслова Евгения": "Суслова Євгенія",
    "Сыромятников Александр": "Сиром'ятников Олександр",
    "Тарасенко Андрей": "Тарасенко Андрій",
    "Тесленко Кирилл": "Тесленко Кирило",
    "Тимченко Михаил": "Тимченко Михайло",
    "Тихонченко Виталий": "Тихонченко Віталій",
    "Ткаченко Сергей": "Ткаченко Сергій",
    "Ткачук Сергей": "Ткачук Сергій",
    "Топко Александр": "Топко Олександр",
    "Топко Андрей": "Топко Андрій",
    "Торский Тарас": "Торський Тарас",
    "Трубецкой Пётр": "Трубецькой Петро",
    "Улезко Анатолий": "Улезко Анатолій",
    "Усетс Терьер": "Усетс Терьєр",
    "Усик Артём": "Усик Артем",
    "Утембаев Андрей": "Утембаєв Андрій",
    "Федотова Ирина": "Федотова Ірина",
    "Федуна Виктор": "Федуна Віктор",
    "Федуна Игорь": "Федуна Ігор",
    "Франчук Андрей": "Франчук Андрій",
    "Харченко Татьяна": "Харченко Тетяна",
    "Химко Александр": "Хімко Олександр",
    "Химчак Сергей": "Хімчак Сергій",
    "Ходеев Николай": "Ходєєв Микола",
    "Ходяков Владимир": "Ходяков Володимир",
    "Хоменко Владимир": "Хоменко Володимир",
    "Худяшов Станислав": "Худяшов Станіслав",
    "Хусаинов Сергей": "Хусаінов Сергій",
    "Царолунга Дмитрий": "Царолунга Дмитро",
    "Чабанова": "Чабанова Анна",
    "Черватюк Ирина": "Черватюк Ірина",
    "Червинская Нина": "Червинська Ніна",
    "Черноброва Александра": "Черноброва Олександра",
    "Чижмадия Томаш": "Чижмадія Томаш",
    "Чик Эдуард": "Чик Едуард",
    "Чумак Анна": "Чумак Ганна",
    "Чумак Дмитрий": "Чумак Дмитро",
    "Чёрный Виталий": "Чорний Іван",
    "Швец Андрей": "Швець Андрій",
    "Шевченко Сергей": "Шевченко Сергій",
    "Шемет Наталя": "Шемет Наталія",
    "Шинкаренко Василий": "Шинкаренко Василь",
    "Шипицин Олег": "Шипіцин Олег",
    "Шипулин Алексей": "Шипулін Олексей",
    "Шипулин Сергей": "Шипулін Сергій",
    "Ширяев Дмитрий": "Ширяєв Дмитро",
    "Шкуров Артём": "Шкуров Артем",
    "Шмидтке Анджей": "Шмідтке Анджей",
    "Штурко Дмитрий": "Штурко Дмитро",
    "Шумилов Дмитрий": "Шумілов Дмитро",
    "Шумилова Ирина": "Шумілова Ірина",
    "Шуплецов Станислав": "Шуплєцов Станислав",
    "Шуплецова Зоя": "Шуплєцова Зоя",
    "Эриксон Хенинг": "Еріксон Хенінг",
    "Юша Алексей": "Юша Олексій",
    "Якименко Дмитрий": "Якименко Дмитро",
    "Якименко Екатерина": "Якименко Катерина",
    "Якимов Павел": "Якімов Павло",
    "Яненко Сергей": "Яненко Сергій",
    "Яскевич Алёна": "Яскевич Олена",
    "Бурмака Павло": "Кумовицький Павло",
    "Гончаренко Вадим": "Братченко Вадим",
    "Рудковський Василь": "Дідов Василь",
    "Тфмолов Роман": "Тямолов Роман",
    "Бабанин Дмитрий": "Бабанін Дмитро",
    "Агневщиков Юрий": "Агневщиков Юрій",
    # --- Виправлення з другого раунду перевірки Excel (602 імена) ---
    "Букетова Іра": "Букетова Iрина",
    "Гончаренко": "Братченко Вадим",
    "Кара Евген": "Кара Євген",
    "Лях Світлана": "Лях Свiтлана",
    "МорозовСергій": "Морозов Сергій",
    "Мєльтєва Iрина": "Мельтєва Ірина",
    "Полтавец Свiтлана": "Полтавець Світлана",
    "Соколова Вiкторiя": "Соколова Вікторія",
    "Стецишин Виталий": "Стецишин Віталій",
    "Тарас Мильо": "Мильо Тарас",
    "Цебратенко": "Цебратенко Андрій",
    "Цебратенко Андрей": "Цебратенко Андрій",
    "Чабанова Анна": "Чабанова Ганна",
    "Шаблій Іґор": "Шаблій Ігор",
    # --- Виправлення з ручної перевірки Excel-таблиці (911 імен) ---
    "SOL Deen": "Фатех Дін",
    "Євген Каропкін": "Коропкін Євген",
    "Євген Коропкін": "Коропкін Євген",
    "Євген Шапка": "Шапка Євген",
    "Іван Білоус": "Білоус Іван",
    "Іван Пішта": "Пішта Іван",
    "Ігнатюк": "Ігнатюк Наталія",
    "Ігор Шаблій": "Шаблій Ігор",
    "Ірина Букетова": "Букетова Ірина",
    "Ірина Кива": "Кива Ірина",
    "Ірина Мєльтєва": "Мельтєва Ірина",
    "Ірина Томків": "Томків Ірина",
    "Акользин Дмитро": "Акользін Дмитро",
    "Акользин": "Акользін Дмитро",
    "Чик Едуард": "Чік Едуард",
    "Чабанова Ганна": "Чабанова Анна",
    "Анна Чабанова": "Чабанова Анна",
    "Ганна Чабанова": "Чабанова Анна",
    "Квиклене Рената": "Квіклене Рената",
    "Білодід Крістіна": "Білодід Христина",
    "Альона Ананенко": "Ананенко Альона",
    "Альона Штувбейна": "Штувбейна Альона",
    "Анастасія Капран": "Капран Анастасія",
    "Анастасія Скуртова": "Скуртова Анастасія",
    "Анатолій Ізосімом": "Ізосімом Анатолій",
    "Андрій Зиков": "Зиков Андрій",
    "Андрій Плакса": "Плакса Андрій",
    "Андрій Подкопаєв": "Подкопаєв Андрій",
    "Андрій Сєргєєв": "Сєргєєв Андрій",
    "Андрій Цебратенко": "Цебратенко Андрій",
    "Антон Овелько": "Овелько Антон",
    "Антон Цибух": "Цибух Антон",
    "Артем Мороз": "Мороз Артем",
    "Артем Нечипуренко": "Нечипуренко Артем",
    "Арійчук": "Арійчук Олександр",
    "Бабанiн Дмитро": "Бабанін Дмитро",
    "Барсуков": "Барсуков Віталій",
    "Бейреш Ласлов": "Бейреш Ласло",
    "Березин": "Березін Руслан",
    "Бублик": "Бублик В'ячеслав",
    "Бублик Вячеслав": "Бублик В'ячеслав",
    "Бублік Вячеслав": "Бублик В'ячеслав",
    "Бурмака": "Кумовицький Павло",
    "Білоус": "Білоус Іван",
    "В'ячеслав Бублик": "Бублик В'ячеслав",
    "Вадим Гончаренко": "Братченко Вадим",
    "Вадім Гончаренко": "Братченко Вадим",
    "Валерій Бондаренко": "Бондаренко Валерій",
    "Валерій Куча": "Куча Валерій",
    "Ванда Євгеній": "Ванда Євген",
    "Василь Чепак": "Чепак Василь",
    "Владислав Ясинський": "Ясинський Владислав",
    "Володимир Варивода": "Варивода Володимир",
    "Володимир Василевський": "Василевський Володимир",
    "Володимир Мартов": "Мартов Володимир",
    "Володимир Плакса": "Плакса Володимир",
    "Володимир Соколов": "Соколов Володимир",
    "Волощук Олег": "Олег Волощук",
    "Вячеслав Бублик": "Бублик В'ячеслав",
    "Віталій Богдан": "Богдан Віталій",
    "Віталій Кулик": "Кулик Віталій",
    "Віталій Чорний": "Чорний Віталій",
    "Гавриленко Влад": "Гавриленко Владислав",
    "Григоренко": "Григоренко Віктор",
    "Гринів": "Гринів Олександр",
    "Гринів Ю": "Гринів Юрій",
    "Дарʼя Радіонова": "Радіонова Дарʼя",
    "Денис Антіпов": "Антіпов Денис",
    "Денис Пламада": "Пламада Денис",
    "Десинский": "Десинський Максим",
    "Дмитро Акользин": "Акользін Дмитро",
    "Дмитро Акользін": "Акользін Дмитро",
    "Дмитро Бабанін": "Бабанін Дмитро",
    "Дмитро Баштаннік": "Баштаннік Дмитро",
    "Дмитро Кива": "Кива Дмитро",
    "Дмитро Скуртов": "Скуртов Дмитро",
    "Дін": "Фатех Дін",
    "Едуард Калініченко": "Калініченко Едуард",
    "Едуард Чепурний": "Чепурний Едуард",
    "Зарайский": "Зарайский Володимир",
    "Зуев Олег": "Зуєв Олег",
    "Ковальов": "Ковальов Андрій",
    "Коломієць": "Коломієць Андрій",
    "Констянтин Халєєв": "Халеєв Костянтин",
    "КорпусовАндрій": "Корпусов Андрій",
    "Костянтин Буравльов": "Буравльов Костянтин",
    "Костянтин Мелах": "Мелах Костянтин",
    "Кузовкин": "Кузовкін Сергій",
    "Кумовіцький Павло": "Кумовицький Павло",
    "Кільчицький Олксандр": "Кільчицький Олександр",
    "Лагутін": "Лагутін Андрій",
    "ЛюдмилаПішта": "Пішта Людмила",
    "Лілія Панасюк": "Панасюк Лілія",
    "Максим Мутило": "Мутило Максим",
    "Максим Пивошенко": "Пивошенко Максим",
    "Максим Телятник": "Телятник Максим",
    "Максим Ферар": "Ферар Максим",
    "Матвіішин Михайло": "Матвіїшин Михайло",
    "Микита Солтис": "Солтис Микита",
    "Микола Кондарюк": "Кондарюк Микола",
    "Мойсеєнков Дмитрій": "Мойсеєнков Дмитро",
    "Молдаван Емілія": "Молдован Емілія",
    "Молдован": "Молдован Емілія",
    "Молдован В": "Молдован Вероніка",
    "Молдован Е": "Молдован Емілія",
    "Мороз": "Мороз Артем",
    "Морозов": "Морозов Сергій",
    "Наталія Ігнатюк": "Ігнатюк Наталія",
    "Наталія Попова": "Попова Наталія",
    "Нелля Іргізова": "Іргізова Нелля",
    "Окуліцький Євгеній": "Окуліцький Євген",
    "Олег Зуєв": "Зуєв Олег",
    "Олег Матус": "Матус Олег",
    "Олександр Арійчук": "Арійчук Олександр",
    "Олександр Вонхольський": "Вонхольський Олександр",
    "Олександр Дрєєв": "Дрєєв Олександр",
    "Олександр Капран": "Капран Олександр",
    "Олександр Сарія": "Сарія Олександр",
    "Олександр Сидоренко": "Сидоренко Олександр",
    "Олександр Сєлєзньов": "Сєлєзньов Олександр",
    "Олександр Шум": "Шум Олександр",
    "ОлексійПанченко": "Панченко Олексій",
    "Олена Зволякевич": "Зволякевич Олена",
    "Олена Коломієць": "Коломієць Олена",
    "Ольга Пермякова": "Пермякова Ольга",
    "Омельченко": "Омельченко Владислав",
    "Павленко": "Павленко Сергій",
    "Павленко СЕргій": "Павленко Сергій",
    "Павло Бурмака": "Кумовицький Павло",
    "Павло Кумовицкий": "Кумовицький Павло",
    "Павло Кумовицький": "Кумовицький Павло",
    "Пекарук": "Пекарук Ілля",
    "Петро Самодій": "Самодій Петро",
    "Пивошенко": "Пивошенко Олена",
    "Писаревський Віталій": "Писаревський Вiталiй",
    "Попова": "Попова Наталія",
    "Попова Наталі": "Попова Наталія",
    "Пішта": "Пішта Іван",
    "Роман Верхоляк": "Верхоляк Роман",
    "Роман Кузьменко": "Кузьменко Роман",
    "Роман Соколов": "Соколов Роман",
    "Ростислав Матійцов": "Матійцов Ростислав",
    "Святослав Подольський": "Подольський Святослав",
    "Сергій Калініченко": "Калініченко Сергій",
    "Сергій Любих": "Любих Сергій",
    "Сергій Морозов": "Морозов Сергій",
    "Сергій Овчаренко": "Овчаренко Сергій",
    "Сергій Павленко": "Павленко Сергій",
    "Сергій Сахаров": "Сахаров Сергій",
    "Сергій Усик": "Усик Сергій",
    "Сергій Шевченко": "Шевченко Сергій",
    "Сокол Роман": "Соколов Роман",
    "Солтис": "Солтис Микита",
    "Солтис Анна": "Солтис Ганна",
    "Софюк Юлія": "Софлюк Юлія",
    "Сташевский": "Сташевський Валентин",
    "Сєргєєв": "Сергєєв Олексій",
    "Тетяна Онищенко": "Онищенко Тетяна",
    "Тетяна Селіхова": "Селіхова Тетяна",
    "Топчук Анатолiй": "Топчук Анатолій",
    "Трубецький Петро": "Трубецькой Петро",
    "Фатех Дин": "Фатех Дін",
        "Ферар": "Ферар Максим",
    "Халєєв Костянтин": "Халеєв Костянтин",
    "Чепак": "Чепак Василь",
    "Чорний": "Чорний Іван",
    "Шикирюк": "Шикирюк Євген",
    "Шикирюк Евгеній": "Шикирюк Євген",
    "Шклярук": "Шклярук Наталія",
    "Шклярук Наталя": "Шклярук Наталія",
    "Юлія Герасименко": "Герасименко Юлія",
    "Юлія Шклярук": "Шклярук Юлія",
    "Юрій Агневщиков": "Агневщиков Юрій",
    "Юрій Гринів": "Гринів Юрій",
    "Юрій Звягін": "Звягін Юрій",
    "Юрій Севостьянов": "Севостьянов Юрій",
    "Юрій Тарасюк": "Тарасюк Юрій",
    "Юрій Шильцин": "Шильцин Юрій",
    "Ярослав Баширов": "Баширов Ярослав",
    "Ярослав Микитин": "Микитин Ярослав",
}


EXCLUDED_FROM_STATS_NAMES = {
    # Пари/дуети/команди — призерів показуємо (медалі лишаються),
    # але в жодну персональну статистику (Рекорди, середні, H2H) не рахуємо.
    # --- Другий раунд перевірки (602 імена) ---
    "\"rus-нi Реасe-да\"",
    "Andrzej Podgórski",
    "PDC",
    "Ільясова Альбіна\\Лановенко Ксенія",
    "Абчугський Олександр\\Гончаров Едуард",
    "Бурмака/Бублик",
    "Білоус Іван\\Телятник Максим",
    "Білоус Жанна\\Пішта Людмила",
    "Дін Фатех\\Пішта Іван",
    "Заборний Андрій\\Панченко Олексій",
    "Мелашенко Владислав\\Залевський Володимир",
    "Микитин Ярослав\\Стецишин Віталій",
    "Могилевський Руслан\\Пекарук Ілля",
    "Омельченко Владислав\\Гакман Степан",
    "Павленко Сергій\\Чепак Василь",
    "Паламар Оксана\\Ледєньова Наталія",
    "Плакса Володимир\\Плакса Андрій",
    "ПоповаНаталі\\Селіхова Тетяна",
    "Рудковський Василь\\Троценко Артем",
    "Спекторенко Катерина\\Карнаух Валентина",
    "Усик Артем\\Сташевський Валентин",
    "Халєєв Констянтин\\Кондарюк Микола",
    "Харченко Тетяна\\Гончаренко Інна",
    "Царолунга Дмитро\\Царолунга Марк",
    "Цебратенко Андрій\\Лагутін Андрій",
    "Цілуйко Владислав\\Мильо Тарас",
    "Чабанова Анна\\Анна Калмурзаєва",
    "Чепурний Едуард\\Гончаренко Вадим",
    "Шклярук Наталія\\Терещенко Любов",
    "BANDERSHTAT",
    "DartsUnited- Volyn Аrrows",
    "DartsUnited-Volyn&Odesa",
    "Fateh Deen/Селіхова Т.",
    "Friends",
    "GL DARTS LEAGUE",
    "Lemberg 1",
    "MISH-MASH",
    "MVB (The Most Valuable of Bukovina)",
    "No doubt",
    "ODESA SHARKS",
    "PDC (Porter Darts Club)",
    "PDC (PorterDartsClub)",
    "PONINKA",
    "Persistent Arrows",
    "Sol Deen/Карнаух Валентина",
    "Sol/Шикирюк",
    "TRI ARKANA",
    "UBK",
    "UBK - United Bukovina & Kryvbas",
    "UKR dumpling",
    "Wild Wild WEST",
    "rus-нi Реасe-да",
    "Ігнатюк Наталія,  Молдован Вероніка",
    "Ігнатюк Наталія/Пішта Людмила",
    "Ігнатюк/Паламар",
    "Ільясова Альбіна, Білоус Жанна",
    "Ільясова Альбіна\\Лановенко Ксенія",
    "Ільясова/Арійчук",
    "Ільясова/Кондарюк",
    "Ільясова/Лановенко",
    "Абчугський О./Букетова І.",
    "Абчугський Олександр, Кіфор Ярослав",
    "Абчугський Олександр/Головень Юрій",
    "Абчугський Олександр/Ледєньова Наталія",
    "Абчугський Олександр\\Гончаров Едуард",
    "Абчугський/Букетова",
    "Абчугський/Рудковский",
    "Аверін / Лагутін",
    "Аверін Сергій/Троценко Артем",
    "Аверін/Агневщиков",
    "Аверін/Лагутін",
    "Агневщиков Юрій/Зайцева Інна",
    "Агневщиков Юрій/Федуна Ігор",
    "Агневщиков/Абчугський",
    "Ананенко/Ананенко",
    "Арійчук Олександр/Воронюк Михайло",
    "Арійчук Олександр/Лановенко Ксенія",
    "Арійчук Святослав/Аверін Сергій",
    "Арійчук/Григоренко",
    "БАТЯРИ",
    "БФД",
    "Бабанін / Полтавець",
    "Бабанін/Полтавець",
    "Бабанін\\Гончаров",
    "Балаж/Ніщик",
    "Баранов Олександр/Паламар Оксана",
    "Баширов/Шклярук Ю.",
    "Бублик/Стецишин",
    "Буковина",
    "Бурмака Павло/Бублик В'ячеслав",
    "Бурмака/Бублик Л.",
    "Білоус / Білоус",
    "Білоус Іван/Мокан Микола",
    "Білоус Іван\\Телятник Максим",
    "Білоус Данило, Білоус Іван",
    "Білоус Жанна/Ільясова Альбіна",
    "Білоус Жанна\\Пішта Людмила",
    "Білоус.І/Халеєв",
    "Білоус.Ж/Мильо",
    "Білоус/Білоус",
    "Білоус/Пішта",
    "Білоус\\Ільясова",
    "Василь/Гой",
    "Вовки",
    "Воронюк Михайло, Момот Сергій",
    "ГІЛЬДІЯ",
    "ГАБЕЛІ",
    "ГАЛИЦЬКО-ВОЛИНСЬКІ ЛЕВИ",
    "Габелі",
    "Гакман С./Ігнатюк Н.",
    "Гакман Степан/Ігнатюк Наталія",
    "Гакман Степан/Чабанова Анна",
    "Гарбузи",
    "Головень Юрій, Плакса Андрій",
    "Гончаренко Інна, Харченко Тетяна",
    "Гончаренко В./Гончаренко І.",
    "Гончаренко/Гончаренко",
    "Гончаренко/Чепурний",
    "Гончаров Е./Пивошенко О.",
    "Гончаров Едуард/Пивошенко Олена",
    "Гончаров Едуард/Солтис Микита",
    "Гончаров/Агневщиков",
    "Гончаров/Пивошенко",
    "Гоян/Яненко",
    "Григоренко/Ігнатюк",
    "Гринів Олександр/Гринів Юрій",
    "Гринів Олександр/Молдован Вероніка",
    "Данилюк Олександр, Овчаренко Сергій",
    "Данилюк Олександр/Ніщик Володимир",
    "Долганенко Денис, Трубецькой Петро",
    "Долганенко/Долганенко",
    "Долганенко/Попова",
    "Діденко Оксана/Ананенко Альона",
    "Дін Фатех\\Пішта Іван",
    "Заборний Андрій, Панченко Олексій",
    "Заборний Андрій\\Панченко Олексій",
    "Заборний Д/Царолунга М",
    "Заборний/Панченко",
    "Залевський / Мелашенко",
    "Залевський В./Білоус Ж.",
    "Залевський Володимир, Гринів Олександр",
    "Залевський Володимир/Білоус Жанна",
    "Залевський Володимир/Воронюк Михайло",
    "Залевський Володимир/Гринів Олександр",
    "Залевський Володимир/Моісеєнко Віталіна",
    "Залевський/Ігнатюк",
    "Залевський/Білоус",
    "Залевський/Зубаль",
    "Залевський/Мелашенко",
    "Залевський/Чепак",
    "Зубаль Василь, Владика Юрій",
    "Зубаль Василь/Владика Юрій",
    "Зубаль Василь/Шклярук Наталія",
    "Зубаль Василь/Шклярук Юлія",
    "Капран О./Капран А.",
    "Капран/Капран",
    "Капран/Шклярук",
    "Карнаух Валентина, Спекторенко Катерина",
    "Карнаух/Зволякевич",
    "Карнаух\\Спекторенко",
    "Карпевич Олександр/Сапронов Петро",
    "Кива/Кива",
    "Кисельов\\Герман",
    "Козловський Богдан/Пекарук Ілля",
    "Козловський/Кіфор",
    "Коломієць / Шильцин",
    "Коломієць А./Коломієць О.",
    "Коломієць/Коломієць",
    "Кондарюк / Ільясова",
    "Кондарюк Микола/Ільясова Альбіна",
    "Кондарюк Микола/Мильо Тарас",
    "Кондарюк/Ільясова",
    "Кондарюк/Кулик",
    "Кондарюк/Сас",
    "Кондарюк\\Халеєв",
    "Корпусов / Чорний",
    "Корпусов Андрій, Сапронов Петро",
    "Корпусов Андрій/Спекторенко Катерина",
    "Корпусов Андрій/Томчук Олег",
    "Корпусов Андрій/Чорний Іван",
    "Корпусов/Гончаренко",
    "Корпусов/Рудковський",
    "Корпусов/Сапронов",
    "Кулик В./Шклярук Ю.",
    "Кулик Віталій, Рудковський Василь",
    "Кумовицький/Бублик",
    "Кумовіцький/Бублик",
    "Кіфор Я./Ільясова А.",
    "Кіфор Ярослав/Гончаренко Інна",
    "Кіфор/Ільясова",
    "Кіфор/Лановенко",
    "ЛЕВЕШ",
    "Лагутін А./Спекторенко К.",
    "Лагутін Андрій, Цебратенко Андрій",
    "Лагутін Андрій/Діденко Оксана",
    "Лагутін Андрій/Цебратенко Андрій",
    "Лагутін/Карнаух",
    "Лагутін/Спекторенко",
    "Лагутін/Цебратенко",
    "Лагутін\\Цебратенко",
    "Лановенко Ксенія,  Паламар Оксана",
    "Лановенко Ксенія/Терещенко Любов",
    "Лановенко\\Чабанова",
    "Ледєньова Н./Мелашенко В.",
    "Логоша Євген/Бабанін Дмитро",
    "Луков/Лукова",
    "Льохарт Олег, Мокан Микола",
    "Льохарт Олег/Терещенко Любов",
    "Любих С./Зволякевич О.",
    "Любих/Зволякевич",
    "Любих/Карнаух",
    "Любих/Ледєньова",
    "Лях Світлана, Полтавець Світлана",
    "Мартов / Могилевський",
    "Мартов Володимир/Бабанін Дмитро",
    "Мартов/Бабанін",
    "Матвіїшин Михайло/Павленко Ірина",
    "Мелашенко Влад/Павленко Сергій",
    "Мелашенко Владислав, Павленко Сергій",
    "Мелашенко Владислав/Почкай Андрій",
    "Мелашенко Владислав\\Залевський Володимир",
    "Мелашенко/Ледєньова",
    "Мелашенко/Осадча",
    "Мелашенко/Павленко",
    "Мелашенко\\Залевський",
    "Мельник\\Урсакі",
    "Микитин / Кіфор",
    "Микитин Ярослав/Ігнатюк Наталія",
    "Микитин Ярослав/Білоус Іван",
    "Микитин Ярослав/Кіфор Ярослав",
    "Микитин Ярослав/Спекторенко Катерина",
    "Микитин Ярослав\\Стецишин Віталій",
    "Микитин/Кіфор",
    "Микитин/Нечипуренко",
    "Микитин/Павленко",
    "Микитин/Стецишин",
    "Микитин/Чабанова",
    "Мильо Тарас/Білоус Жанна",
    "Мильо Тарас/Чабанова Анна",
    "Митчик/Бойчук",
    "Могилевський Р./Лановенко К.",
    "Могилевський Руслан, Сташевський Валентин",
    "Могилевський Руслан/Лановенко Ксенія",
    "Могилевський Руслан/Селіхова Тетяна",
    "Могилевський Руслан/Сташевський Валентин",
    "Могилевський Руслан\\Пекарук Ілля",
    "Могилевський/Лановенко",
    "Могилевський/Пекарук",
    "Могилевський/Солтис",
    "Могилевський/Чепурний",
    "Момот Сергій/Хоменко Владислав",
    "Момот\\Головень",
    "Мороз / Чепурний",
    "Мороз Артем/Чепурний Едуард",
    "Мороз/Чепурний",
    "Морозов/Бурмака",
    "Моісеєнко/Троценко",
    "Назаренко О./Назаренко О.",
    "Науменко Віталій, Корб'як Ігор",
    "Овелько/Паламар",
    "Овелько/Подольчук",
    "Овсієнко Євген/Абчугський Олександр",
    "Овсієнко Євген/Антіпов Іван",
    "Овсієнко Євген/Паламар Оксана",
    "Овсієнко\\Абчугський",
    "Овчаренко\\Царолунга",
    "Окуліцький Євгеній/Сапронов Петро",
    "Олександр Гринів/Хоменко Владислав",
    "Омельченко / Троценко",
    "Омельченко Баширов",
    "Омельченко В./Терещенко Л.",
    "Омельченко Владислав /Баширов Ярослав",
    "Омельченко Владислав, Гакман Степан",
    "Омельченко Владислав/Ананенко Сергій",
    "Омельченко Владислав/Гакман Степан",
    "Омельченко Владислав/Карнаух Валентина",
    "Омельченко Владислав\\Гакман Степан",
    "Омельченко/Григоренко",
    "Омельченко/Терещенко",
    "Омельченко/Троценко",
    "Омельченко\\Гакман",
    "Павленко / Чепак",
    "Павленко С./Паламар О.",
    "Павленко Сергій/Топчу Владислав",
    "Павленко Сергій\\Чепак Василь",
    "Павленко/Гончаренко",
    "Павленко\\Білоус",
    "Паламар Оксана/Зайцева Інна",
    "Паламар Оксана\\Ледєньова Наталія",
    "Паламар/Заборний",
    "Паламар/Шум",
    "Паламар\\Зайцева",
    "Панасюк Лілія/Ніщик Юліана",
    "Панасюк С./Панасюк Л.",
    "Панасюк Сергій/Балаж Людвик",
    "Панасюк/Ніщик",
    "Панасюк\\Ніщик",
    "Панченко Олексій/Бабанін Дмитро",
    "Панченко\\Заборний",
    "Пекарук І./Чабанова А.",
    "Пекарук/Сидоренко",
    "Пекарук/Соколова",
    "Пекарук/Спекторенко",
    "Пенчук Василь/Плакса Володимир",
    "Пивошенко Максим/Ганечко Марина",
    "Писаревський\\Рукавишников",
    "Плакса / Плакса",
    "Плакса Андрій/Попова Наталі",
    "Плакса Андрій/Пішта Іван",
    "Плакса В./Мильо",
    "Плакса Володимир, Крутій Володимир",
    "Плакса Володимир/Фісяк Павло",
    "Плакса Володимир\\Плакса Андрій",
    "Плакса.А/Селіхова",
    "Плакса.В/Царолунга",
    "Плакса/Гакман",
    "Плакса/Плакса",
    "Плакса\\Плакса",
    "Пламада / Григоренко",
    "Пламада/Григоренко",
    "Покладок / Ющик",
    "Покладок Олег/Бадира Олександр",
    "Покладок/Ющик",
    "Попова Наталі, Селіхова Тетяна",
    "Попова/Селіхова",
    "ПоповаНаталі\\Селіхова Тетяна",
    "Почкай Андрій/Агневщиков Юрій",
    "Почкай Андрій/Почкай Маріанна",
    "Приходько/Форманюк",
    "Пішта.І/Панченко",
    "Пішта.Л/Гакман",
    "Пішта/Панасюк",
    "Пішта/Пішта",
    "Разінков",
    "Руденко",
    "Рудковский/Шклярук",
    "Рудковський В./Шклярук Н.",
    "Рудковський Василь\\Троценко Артем",
    "Сакура",
    "Селіхова\\Попова",
    "Сидоренко/Шклярук Н.",
    "Силюк/Савчук",
    "Снайперки",
    "Солтис М./Солтис А.",
    "Солтис Микита, Бабанін Дмитро",
    "Солтис Микита/Коломієць Андрій",
    "Солтис Микита/Терещенко Любов",
    "Солтис/Солтис",
    "Спекторенко Катерина/Карнаух Валентина",
    "Спекторенко Катерина\\Карнаух Валентина",
    "Сташевський Валентин/Харченко Тетяна",
    "Сташевський/SOL",
    "Сташевський/Селіхова",
    "Сташевський\\Могилевський",
    "Стебелько",
    "Стецишин Віталій, Мильо Тарас",
    "Стецишин Віталій/Кордівський Арсен",
    "Стецишин/Носівець",
    "Телятник/Гринів О.",
    "Топчук\\Караман",
    "Трофімов",
    "Троценко Артем /Лагутін Андрій",
    "Троценко/Лагутін",
    "Троценко/Чабанова",
    "Усик А./Ільясова",
    "Усик А./Усик С.",
    "Усик Артем/Агневщиков Юрій",
    "Усик Артем\\Сташевський Валентин",
    "Усик Сергій/Мєльтєва Ірина",
    "Усик/Кисельов",
    "Усик\\Soul Deen",
    "Фатех Дін, Кондарюк Микола",
    "Фатех/Попова",
    "Ферар/Ферар",
    "Фехтел Анатолій, Мойсеєнков Дмитрій",
    "Форманюк Андрій/Головень Юрій",
    "Халеєв Костянтин, Царолунга Дмитро",
    "Халєєв Констянтин\\Кондарюк Микола",
    "Харченко Тетяна/Гончаренко Інна",
    "Харченко Тетяна\\Гончаренко Інна",
    "Харченко/Гончаренко",
    "Харченко\\Гончаренко",
    "Хуренко Сергій, Зубаль Маркіян",
    "Царолунга Дмитро/Заборний Андрій",
    "Царолунга Дмитро\\Царолунга Марк",
    "Царолунга.М/Заборний.Д",
    "Царолунга/Скуртов",
    "Цебратенко / Долганенко",
    "Цебратенко А./Карнаух В.",
    "Цебратенко Андрій/Гончаренко Інна",
    "Цебратенко Андрій/Шклярук Наталія",
    "Цебратенко Андрій\\Лагутін Андрій",
    "Цебратенко/Долганенко",
    "Цебратенко/Овчаренко",
    "Цебратенко/Терещенко",
    "Цибух",
    "Цілуйко Владислав, Яненко Сергій",
    "Цілуйко Владислав/Логоша Євген",
    "Цілуйко Владислав/Окуліцький Євгеній",
    "Цілуйко Владислав\\Мильо Тарас",
    "Цілуйко\\Микитин",
    "Чабанова Анна\\Анна Калмурзаєва",
    "Чепак Василь, Баранов Олександр",
    "Чепак Василь/Ананенко Альона",
    "Чепак Василь/Мойсеєнков Дмитро",
    "Чепак/Любих",
    "Чепак/Паламар",
    "Чепак/Яворський",
    "Чепак\\Мильо",
    "Чепурний Е./Харченко Т.",
    "Чепурний Едуард, Хоменко Владислав",
    "Чепурний Едуард/Харченко Тетяна",
    "Чепурний Едуард\\Гончаренко Вадим",
    "Чепурний/Спекторенко",
    "Чепурний/Харченко",
    "Чепурний/Чепурна",
    "Чепурний\\Хоменко",
    "Чорний/Гончаров",
    "Чорний/Корпусов",
    "Чубат",
    "Шаблій",
    "Шевчук Дмитро/Приходько Сергій",
    "Шевчук/Томчук",
    "Шильцин/Герасименко",
    "Шклярук Наталія\\Терещенко Любов",
    "Шклярук/Терещенко",
    "Шклярук\\Терещенко",
    "Штувбейна\\Мельниченко",
    "Шум Олександр/Паламар Оксана",
    "Ющик Сергій/Долганенко Денис",
    "Ющик/Яненко",
    "Яворський / Любих",
    "Яворський Костянтин/Лемещук Олександр",
    "Яворський Костянтин/Любих Сергій",
    "Яворський/Бадира",
    "Яненко Сергій/Чепурний Едуард",
}


# Латинські літери, які виглядають ідентично до кириличних — типова помилка
# набору тексту з перемкнутою не тією розкладкою клавіатури. Застосовується
# ПЕРЕД пошуком аліасів у resolve_name(), щоб такі "невидимі" різночитання
# автоматично об'єднувались із правильним кириличним варіантом.
LATIN_TO_CYRILLIC_HOMOGLYPHS = str.maketrans({
    "a": "а", "e": "е", "i": "і", "o": "о", "p": "р", "c": "с", "x": "х", "y": "у",
    "A": "А", "B": "В", "E": "Е", "K": "К", "M": "М", "H": "Н", "O": "О", "P": "Р",
    "C": "С", "T": "Т", "X": "Х", "I": "І",
})


def resolve_name(name, name_index, canonical_names=None, fuzzy_cutoff=0.88):
    """Уніфікує ім'я гравця під наш канонічний формат "Прізвище Ім'я".

    Найважливіше правило: ІМ'Я НІКОЛИ не виправляємо нечітко — тільки
    точний збіг. Короткі різні імена (напр. "Марина"/"Арина") можуть мати
    оманливо високу формальну схожість рядка, хоча це різні люди. Нечітким
    може бути тільки ПРІЗВИЩЕ (одруківки на кшталт "Мгилевський", або
    українська/російська різниця написання на кшталт "Могилевский") — і
    тільки коли ім'я збігається ТОЧНО."""
    if not name:
        return name
    # .strip() прибирає пробіли лише на краях. Деякі джерела (особливо
    # старий архів рейтингів) інколи мають ПОДВІЙНІ пробіли всередині імені —
    # без нормалізації такий рядок не співпаде з жодним аліасом точно.
    candidate = re.sub(r"\s+", " ", name.strip())
    # Латинські літери-двійники кирилиці (виглядають ідентично, але це інші
    # символи для комп'ютера — типова помилка при наборі з іншою розкладкою).
    # Застосовуємо ЛИШЕ якщо рядок уже містить кирилицю — інакше зламали б
    # справді іноземні імена (Sol Deen, Andrzej Podgórski тощо).
    if re.search(r"[а-яіїєґА-ЯІЇЄҐ]", candidate):
        candidate = candidate.translate(LATIN_TO_CYRILLIC_HOMOGLYPHS)

    if candidate in MANUAL_NAME_ALIASES:
        return MANUAL_NAME_ALIASES[candidate]

    parts = candidate.split()

    # Одне слово (тільки прізвище) — НЕ розгортаємо автоматично. Наша база
    # відомих імен (name_index) будується лише з медалістів таблиці
    # "Призери" — вона не знає про кожного реального гравця. Якщо в родині
    # двоє гравців з однаковим прізвищем (напр. Пивошенко Олена й Максим),
    # а медалі є тільки в одного з них — "однозначність" у НАШІЙ базі не
    # означає однозначність У РЕАЛЬНОСТІ. Це реально трапилось і помилково
    # приписало матчі Максима Олені. Краще лишити голе прізвище як є, ніж
    # ризикувати підмінити особу (і навіть стать).
    if len(parts) == 1:
        return candidate

    if len(parts) == 2:
        first_word, second_word = parts

        if candidate in (canonical_names or []):
            return candidate

        if canonical_names:
            import difflib
            # Пряме зіставлення: "Прізвище Ім'я" — ім'я (second_word) має
            # збігатись ТОЧНО, прізвище (first_word) може бути одруківкою.
            for cname in canonical_names:
                cparts = cname.split()
                if len(cparts) != 2:
                    continue
                csurname, cfirst = cparts
                if cfirst == second_word:
                    if difflib.SequenceMatcher(None, first_word, csurname).ratio() >= fuzzy_cutoff:
                        return cname

            # Порядок слів переплутано ("Ім'я Прізвище") — тут ім'я вже у
            # позиції first_word, теж має збігатись ТОЧНО.
            for cname in canonical_names:
                cparts = cname.split()
                if len(cparts) != 2:
                    continue
                csurname, cfirst = cparts
                if cfirst == first_word:
                    if difflib.SequenceMatcher(None, second_word, csurname).ratio() >= fuzzy_cutoff:
                        return cname

    return candidate




def extract_podium_tpids(stats):
    """Повертає {1: tpid, 2: tpid, 3: tpid}. Спершу пробує поле rank (працює
    для турнірів на вибування). Якщо rank ніде не проставлено (типово для
    групових/round-robin турнірів — часто трапляється в командних форматах),
    інферимо місця самі: за перемогами в матчах, потім у сетах, потім у легах."""
    podium = {}
    for tpid, stat in stats.items():
        rank = stat.get("rank")
        if rank in (1, 2, 3):
            podium[rank] = tpid
    if podium:
        return podium

    ranked = sorted(
        stats.items(),
        key=lambda kv: (kv[1].get("winMatch", 0), kv[1].get("winSet", 0), kv[1].get("winLeg", 0)),
        reverse=True,
    )
    return {i + 1: tpid for i, (tpid, _) in enumerate(ranked[:3])}


def split_medals_by_gender(nakka_data, name_index, canonical_names, known_women, known_men, default_gender):
    """Визначає стать КОЖНОГО призера окремо за відомим списком імен, а не
    за тим, з якої колонки (Men/Women) прийшло посилання. Це критично для
    Мікст/Пар/Команд, де Nakka часто веде ОДНУ спільну сітку на обидві
    статі — раніше такі турніри показували лише "чоловічих" призерів,
    бо посилання лежало в колонці Men."""
    if not nakka_data:
        return None, None
    entries, stats = nakka_data["entries"], nakka_data["stats"]
    podium_tpids = extract_podium_tpids(stats)
    podium_men, podium_women = {}, {}
    for rank, tpid in podium_tpids.items():
        name = resolve_name(entries.get(tpid, tpid), name_index, canonical_names)
        if name in known_women:
            podium_women[rank] = name
        elif name in known_men:
            podium_men[rank] = name
        elif default_gender == "women":
            podium_women[rank] = name
        else:
            podium_men[rank] = name

    def to_medal(podium):
        if not podium:
            return None
        return {"gold": podium.get(1), "silver": podium.get(2), "bronze": [podium[3]] if podium.get(3) else []}

    return to_medal(podium_men), to_medal(podium_women)


def fetch_match_averages(tdid, cache):
    """Тягне список матчів турніру (match/list вже включає statsData за
    замовчуванням — окремий запит на кожен матч не потрібен) і рахує
    середній КОЖНОГО ОКРЕМОГО МАТЧУ для кожного гравця."""
    if tdid in cache:
        return cache[tdid]

    matches = []
    skip = 0
    try:
        while True:
            url = f"{NAKKA_API_BASE}/match/list?tdid={tdid}&endMatch=1&count=100&skip={skip}"
            resp = fetch_json(url)
            time.sleep(0.05)
            if resp.get("result") != 0:
                break
            batch = resp.get("list", [])
            matches.extend(batch)
            if len(batch) < 100 or skip > 500:  # запобіжник від нескінченної пагінації
                break
            skip += 100
    except Exception as e:
        print(f"    match/list fetch failed for {tdid}: {e}")

    cache[tdid] = matches
    return matches


def apply_manual_medal_overrides_final(tournaments):
    """Застосовує ручні перевизначення призерів ОСТАННІМ кроком — вже ПІСЛЯ
    enrich_with_nakka(). Це критично: enrich_with_nakka() записує в
    nakkaMedals реальні (та часто короткі, без складу) дані прямо з Nakka,
    і фронтенд віддає перевагу саме nakkaMedals над medals, коли обидва є.
    Без цього фінального проходу ручні виправлення (напр. повний склад
    команди) залишались би "невидимими" — технічно записаними в data.json,
    але затуленими автоматичними даними Nakka."""
    updated = 0
    for t in tournaments:
        override = MANUAL_MEDAL_OVERRIDES.get((t["date"], t["name"])) or MANUAL_MEDAL_OVERRIDES_BY_DATE.get(t["date"])
        if not override:
            continue
        if "medals" in override:
            t["medals"] = override["medals"]
            t["nakkaMedals"] = override["medals"]
        if "medalsWomen" in override:
            t["medalsWomen"] = override["medalsWomen"]
            t["nakkaMedalsWomen"] = override["medalsWomen"]
        updated += 1
    if updated:
        print(f"  Фінально перевизначено призерів (включно з nakkaMedals) для {updated} турнірів")


def enrich_with_nakka(tournaments, name_index, canonical_names=None, known_women=None, known_men=None):
    """Проходить по всіх турнірах, тягне Nakka tdid з посилань, і додає
    t['nakkaMedals'] / t['nakkaMedalsWomen'] (надійні призери напряму з API)
    плюс повертає плаский список усіх гравець-турнір записів статистики
    (сировина для секції "Рекорди" — топ по середньому, 180-ках тощо) і
    список середніх ПО КОЖНОМУ ОКРЕМОМУ МАТЧУ (для рекорду "середній за матч")."""
    cache = {}
    match_cache = {}
    player_records = []
    match_records = []
    h2h_records = []
    fetched = 0
    known_women = known_women or set()
    known_men = known_men or set()

    for t in tournaments:
        links = t.get("links", {})

        def link_tdid(*keys):
            for k in keys:
                link = links.get(k)
                if link and link.get("type") == "nakka":
                    tdid = extract_tdid(link["url"])
                    if tdid:
                        return tdid
            return None

        men_tdid = link_tdid("men", "menAvg")
        women_tdid = link_tdid("women", "womenAvg")
        other_tdid = None
        if not men_tdid and not women_tdid:
            other_tdid = link_tdid("tournament")

        t["nakkaMedals"] = None
        t["nakkaMedalsWomen"] = None

        for tdid, default_gender in (
            (men_tdid, "men"),
            (women_tdid, "women"),
            (other_tdid, "men"),
        ):
            if not tdid:
                continue
            data = fetch_nakka_tournament(tdid, cache)
            fetched += 1
            if not data:
                continue

            medals_men, medals_women = split_medals_by_gender(
                data, name_index, canonical_names, known_women, known_men, default_gender
            )
            if medals_men and not t["nakkaMedals"]:
                t["nakkaMedals"] = medals_men
            if medals_women and not t["nakkaMedalsWomen"]:
                t["nakkaMedalsWomen"] = medals_women

            if tdid in EXCLUDED_STATS_TDIDS:
                # Призерів (вище) лишаємо, а от у жодну статистику (Рекорди,
                # середні, H2H тощо) цей турнір не потрапляє — дані биті.
                continue

            # "Команди" і "Пари" виключаємо повністю з БУДЬ-ЯКОЇ статистики
            # (не тільки з середнього за матч) — там "сторона"/"гравець" у
            # Nakka може бути командою чи змішаною парою ("Kyiv Bulls",
            # "Залевський/Чепак" тощо), а не однією людиною. Раніше це
            # виключалось лише для матчів, а базова турнірна статистика
            # (найкращий лег, 180-ки, checkout) досі мала биті записи.
            format_and_name = (t.get("format", "") + " " + t.get("name", "")).lower()
            is_excluded_format = "команди" in format_and_name or "пари" in format_and_name
            if is_excluded_format:
                continue

            for tpid, stat in data["stats"].items():
                avg = player_avg(stat)
                if avg is None:
                    continue  # гравець не зіграв жодного дротика — пропускаємо
                name = resolve_name(data["entries"].get(tpid, tpid), name_index, canonical_names)
                if name in EXCLUDED_FROM_STATS_NAMES:
                    continue  # пара/команда — рахуємо в призерах, але не в персональній статистиці
                if name in known_women:
                    gender = "women"
                elif name in known_men:
                    gender = "men"
                else:
                    gender = default_gender
                player_records.append({
                    "name": name,
                    "gender": gender,
                    "isUDL": t["isUDL"],
                    "date": t["date"],
                    "tournament": t["name"],
                    "city": t["city"],
                    "avg": avg,
                    "ton80": stat.get("ton80", 0),
                    "highOutCount": stat.get("highOutCount", 0),
                    "highOut": stat.get("highOut", 0),
                    "bestLeg": stat.get("best", 0),
                    "rank": stat.get("rank", 0),
                    "match": stat.get("match", 0),
                    "winMatch": stat.get("winMatch", 0),
                })

            # Фаза 1: середній за окремий матч (match/list вже дає statsData,
            # без потреби в окремому запиті на кожен матч).
            matches = fetch_match_averages(tdid, match_cache)
            for m in matches:
                stats_data = m.get("statsData") or []
                if len(stats_data) != 2:
                    continue
                match_round = (m.get("title") or "").strip()

                resolved_sides = []
                for i, side in enumerate(stats_data):
                    all_score = side.get("allScore") or 0
                    all_darts = side.get("allDarts") or 0
                    if all_darts <= 0:
                        resolved_sides = []
                        break
                    match_avg = round(all_score / all_darts * 3, 2)
                    name = resolve_name(side.get("name") or "", name_index, canonical_names)
                    if name in EXCLUDED_FROM_STATS_NAMES:
                        resolved_sides = []
                        break  # пара/команда — весь матч не рахуємо в персональну статистику
                    if name in known_women:
                        gender = "women"
                    elif name in known_men:
                        gender = "men"
                    else:
                        gender = default_gender
                    resolved_sides.append({
                        "name": name, "gender": gender, "avg": match_avg,
                        "winSets": side.get("winSets") or 0,
                        "winLegs": side.get("winLegs") or 0,
                    })

                if len(resolved_sides) != 2:
                    continue  # хтось не кинув жодного дротика — не рахуємо цей матч

                p1, p2 = resolved_sides
                # Рахунок матчу: спершу сети, якщо формат безсетовий (0-0) — леги.
                if p1["winSets"] or p2["winSets"]:
                    score1, score2 = p1["winSets"], p2["winSets"]
                else:
                    score1, score2 = p1["winLegs"], p2["winLegs"]

                # ВФД завжди грає до мінімум 3 перемог — рахунок переможця
                # менше 3 означає незавершений/аномальний матч (обрив
                # зв'язку, технічна поразка тощо). Такі матчі спотворюють і
                # "середній за матч", і H2H — виключаємо їх повністю.
                if max(score1, score2) < 3:
                    continue

                result = f"{score1}-{score2}"

                for i, side in enumerate(resolved_sides):
                    opponent = resolved_sides[1 - i]
                    match_records.append({
                        "name": side["name"],
                        "gender": side["gender"],
                        "isUDL": t["isUDL"],
                        "date": t["date"],
                        "tournament": t["name"],
                        "city": t["city"],
                        "opponent": opponent["name"],
                        "avg": side["avg"],
                    })

                # Head-to-head: один запис на матч (обидва гравці разом) —
                # сировина для "H2H з найкращим AVG" (найвищий середній АВГ обох).
                h2h_records.append({
                    "name1": p1["name"], "avg1": p1["avg"],
                    "name2": p2["name"], "avg2": p2["avg"],
                    "avgAvg": round((p1["avg"] + p2["avg"]) / 2, 2),
                    "result": result,
                    "gender": p1["gender"],  # обидва гравці одного матчу — одна стать
                    "isUDL": t["isUDL"],
                    "date": t["date"],
                    "tournament": t["name"],
                    "format": t.get("format", ""),
                    "round": match_round,
                    "city": t["city"],
                })

    print(f"  Fetched {fetched} Nakka tournament records ({len(cache)} unique tdid, "
          f"{sum(1 for v in cache.values() if v)} succeeded)")
    print(f"  Fetched match-level averages: {len(match_records)} rows from "
          f"{sum(len(v) for v in match_cache.values())} matches across {len(match_cache)} tdid")
    # Другий прохід: добудовуємо голі прізвища (без імені), яких лишилось
    # багато після того, як ми прибрали ризиковане розгортання "наосліп".
    # Тепер робимо це БЕЗПЕЧНІШЕ: збираємо повні імена не тільки з медалістів
    # (canonical_names), а з УСІХ імен, які реально зустрілись у статистиці
    # цього ж запуску — і розгортаємо голе прізвище, лише якщо серед УСІХ
    # цих джерел воно теж лишається однозначним (рівно один варіант).
    from collections import defaultdict
    surname_variants = defaultdict(set)
    for full_name in canonical_names:
        parts = full_name.split()
        if len(parts) == 2:
            surname_variants[parts[0]].add(full_name)
    for rec in player_records:
        parts = rec["name"].split()
        if len(parts) == 2:
            surname_variants[parts[0]].add(rec["name"])

    expanded_index = {s: next(iter(v)) for s, v in surname_variants.items() if len(v) == 1}

    def backfill(rec, *fields):
        for field in fields:
            val = rec.get(field)
            if val and len(val.split()) == 1 and val in expanded_index:
                rec[field] = expanded_index[val]

    backfilled = 0
    for rec in player_records:
        before = rec["name"]
        backfill(rec, "name")
        if before != rec["name"]:
            backfilled += 1
    for rec in match_records:
        backfill(rec, "name", "opponent")
    for rec in h2h_records:
        backfill(rec, "name1", "name2")

    # Той самий безпечний backfill — і для медалей у таблиці "Турніри"
    # (nakkaMedals/medals), інакше там і далі лишались би голі прізвища
    # навіть коли ми вже знаємо повне ім'я з іншого турніру цього ж запуску.
    medals_backfilled = 0
    for t in tournaments:
        for key in ("medals", "medalsWomen", "nakkaMedals", "nakkaMedalsWomen"):
            medal = t.get(key)
            if not medal:
                continue
            before_gold, before_silver = medal.get("gold"), medal.get("silver")
            if medal.get("gold"):
                backfill(medal, "gold")
            if medal.get("silver"):
                backfill(medal, "silver")
            if medal.get("bronze"):
                medal["bronze"] = [
                    expanded_index.get(n, n) if n and len(n.split()) == 1 else n
                    for n in medal["bronze"]
                ]
            if medal.get("gold") != before_gold or medal.get("silver") != before_silver:
                medals_backfilled += 1
    if backfilled:
        print(f"  Добудовано ім'я для {backfilled} записів (голе прізвище -> повне ім'я, безпечно)")
    if medals_backfilled:
        print(f"  Добудовано ім'я для {medals_backfilled} медальних заліків турнірів")

    return player_records, match_records, h2h_records


# ---------------------------------------------------------------------------
# Для турнірів до появи Nakka в Україні (немає посилання-сітки) єдине
# джерело призерів — протокол (Google Docs). Повний текст документа нам не
# потрібен: Google сам генерує короткий опис (og:description) із перших
# рядків файлу, а туди зазвичай виносять саме підсумкову таблицю місць,
# на кшталт "1 Усик Артем (Київ)2 Мамика Олександр (Кривий Ріг)3 ...".
# ---------------------------------------------------------------------------
import html as _html_module

PROTOCOL_ENTRY_RE = re.compile(r"(\d+)\s*([^\d()]+?)\s*\(([^)]+)\)")


def parse_protocol_description(desc):
    """Читає рядки "1 Ім'я (Місто)2 Ім'я (Місто)..." з опису документа.
    Зупиняється, як тільки порядок рангів переривається (наприклад "5-8") —
    це означає, що далі йде групове місце, а не персональний подіум."""
    podium = []
    expected = 1
    for rank_str, name, _city in PROTOCOL_ENTRY_RE.findall(desc):
        if rank_str != str(expected):
            break
        podium.append(name.strip())
        expected += 1
        if expected > 4:  # більше нам і не треба (золото/срібло/2×бронза)
            break
    return podium


def fetch_protocol_podium(url):
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (vfd-darts-sync)"})
        with urllib.request.urlopen(req, timeout=20) as resp:
            content = resp.read().decode("utf-8", errors="ignore")
    except Exception as e:
        print(f"    protocol fetch failed for {url}: {e}")
        return None

    m = re.search(r'<meta property="og:description" content="([^"]*)"', content)
    if not m:
        return None
    desc = _html_module.unescape(m.group(1))
    podium = parse_protocol_description(desc)
    return podium or None


MANUAL_MEDAL_OVERRIDES = {
    # Ключ: (дата, назва турніру) — точні перевизначення за протоколом, коли
    # автоматичне зіставлення рік+етап дало неправильний результат (напр.
    # у той рік було кілька "ЧУ"-подій і зіставилась не та).
    ("24.06.2017", "ЧУ 2017 DIDO"): {
        "medals": {
            "gold": "Григоренко Денис",
            "silver": "Чумак Дмитро",
            "bronze": ["Лагутін Андрій"],
        },
        "medalsWomen": {
            "gold": "Чумак Ганна",
            "silver": "Камелькова Людмила",
            "bronze": ["Моісеєнко Віталіна"],
        },
    },
    ("22.05.2021", "ЧУ 2021 Команди"): {
        "medals": {
            "gold": "Київ-1 (Усик Артем, Усик Сергій, Ледєньов Вадим, Морозов Сергій)",
            "silver": "Кривий Ріг (Омельченко Владислав, Солтис Андрій, Солтис Микита, Кулик Віталій)",
            "bronze": ["Черкаси-2 (Марченко Денис, Подольський Святослав, Чепурний Едуард, Мороз Артем)",
                       "Чернівці (Гоян Дмитро, Арійчук Олександр, Козловський Богдан, Залевський Володимир)"],
        },
        "medalsWomen": {
            "gold": "Черкаси (Моісеєнко В. / Соколова В.)",
            "silver": "Київ/Харків (Ледєньова Н. / Лановенко К.)",
            "bronze": ["Київ (Мєльтєва І. / Лях С.)", "Кривий Ріг (Чабанова А. / Спекторенко К.)"],
        },
    },
    ("08.12.2024", "ЧУ 2024 Мікст"): {
        "medals": {
            "gold": "Чепурний Едуард/Харченко Тетяна",
            "silver": "Залевський Володимир/Білоус Жанна",
            "bronze": ["Могилевський Руслан/Лановенко Ксенія"],
        },
        "medalsWomen": None,
    },
    ("20.07.2024", "ЧУ 2024 DIDO"): {
        "medals": {
            "gold": "Залевський Володимир",
            "silver": "Кіфор Ярослав",
            "bronze": ["Гоян Дмитро", "Григоренко Віктор"],
        },
        "medalsWomen": {
            "gold": "Харченко Тетяна",
            "silver": "Лановенко Ксенія",
            "bronze": ["Гончаренко Інна", "Білоус Жанна"],
        },
    },
    ("08.12.2018", "ЧУ 2018 Мікст"): {
        "medals": {
            "gold": "Усик Артем/Ледєньова Наталя",
            "silver": "Ледєньов Вадим/Ільясова Альбіна",
            "bronze": ["Омельченко Владислав/Марфола Оксана"],
        },
        "medalsWomen": None,
    },
    ("08.12.2018", "ЧУ 2018 Юнаки"): {
        "medals": {
            "gold": "Солтис Микита",
            "silver": "Мотуз Єгор",
            "bronze": ["Пекарук Ілля"],
        },
        "medalsWomen": None,
    },
    ("15.09.2019", "KYIV MASTERS YOUNG"): {
        "medals": {
            "gold": "Солтис Микита",
            "silver": "Пекарук Ілля",
            "bronze": ["Тямолов Ростислав"],
        },
        "medalsWomen": None,
    },
    ("14.09.2019", "UKRAINE OPEN YOUNG"): {
        "medals": {
            "gold": "Солтис Микита",
            "silver": "Пекарук Ілля",
            "bronze": ["Тямолов Ростислав"],
        },
        "medalsWomen": None,
    },
    ("15.09.2019", "KYIV MASTERS"): {
        "medals": {
            "gold": "Мамика Олександр",
            "silver": "Субаши Алпер",
            "bronze": ["Рогов Віталій", "Холліуелл Вейн"],
        },
        "medalsWomen": {
            "gold": "Клочек Ксенія",
            "silver": "Моісеєнко Віталіна",
            "bronze": ["Чумак Ганна", "Анісімова Світлана"],
        },
    },
    ("14.09.2019", "UKRAINE OPEN"): {
        "medals": {
            "gold": "Паскару Габріель",
            "silver": "Усик Артем",
            "bronze": ["Стейєр Себастьян", "Грєков Юрій"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Клочек Ксенія",
            "bronze": ["Чумак Ганна", "Солтис Ганна"],
        },
    },
}

MANUAL_MEDAL_OVERRIDES_BY_DATE = {
    # Ключ: тільки дата — коли точна назва турніру невідома напевно, але на
    # цю дату в базі лише один такий турнір. Звірено вручну з протоколами.
    "15.06.2019": {
        "medals": {
            "gold": "Омельченко Владислав/Мамика Олександр",
            "silver": "Усик Артем/Ледєньов Вадим",
            "bronze": ["Тямолов Роман/Агневщиков Анатолій", "Солтис Андрій/Рогов Віталій"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна/Ледєньова Наталя",
            "silver": "Марфола Оксана/Харченко Тетяна",
            "bronze": ["Пивошенко Олена/Солтис Ганна", "Чумак Ганна/Грушовець Ольга"],
        },
    },
    "08.09.2018": {
        "medals": {
            "gold": "Усик Артем/Ледєньов Вадим",
            "silver": "Бушуй Олексій/Котляр Юрій",
            "bronze": ["Омельченко Владислав/Мамика Олександр"],
        },
        "medalsWomen": {
            "gold": "Редзюк Тетяна/Некрістова Наталія",
            "silver": "Моісеєнко Віталіна/Ледєньова Наталя",
            "bronze": ["Звягіна Анастасія/Ільясова Альбіна"],
        },
    },
    "09.09.2017": {
        "medals": {
            "gold": "Сташевський Валентин/Сохаіл Соул",
            "silver": "Усик Артем/Ледєньов Вадим",
            "bronze": ["Омельченко Владислав/Васюра Андрій"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна/Анісімова Світлана",
            "silver": "Чумак Ганна/Грушовець Ольга",
            "bronze": ["Ледєньова Наталя/Павлюченко Марина"],
        },
    },
    "10.09.2016": {
        "medals": {
            "gold": "Усик Артем/Григоренко Денис",
            "silver": "Бадира/Томчук",
            "bronze": ["Омельченко Владислав/Васюра Андрій", "Агневщиков Юрій/Буханцов Тарас"],
        },
        "medalsWomen": {
            "gold": "Редзюк Тетяна/Ледєньова Наталя",
            "silver": "Моісеєнко Віталіна/Анісімова Світлана",
            "bronze": ["Чумак Ганна/Грушовець Ольга"],
        },
    },
    "12.09.2015": {
        "medals": {
            "gold": "Усик Артем/Ледєньов Вадим",
            "silver": "Савенков Антон/Бутрій Володимир",
            "bronze": ["Алхімов Андрій/Мороз Ігор"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна/Марфола Оксана",
            "silver": "Чумак Ганна/Анісімова Світлана",
            "bronze": ["Потан Лілія/Калаур Інна"],
        },
    },
    "18.05.2019": {
        "medals": {
            "gold": "Кривий Ріг-1 (Мамика Олександр, Омельченко Владислав, Рогов Віталій, Солтис Андрій)",
            "silver": "Київ-1 (Усик Артем, Ледєньов Вадим, Некрістов Станіслав, Ромашко Віталій)",
            "bronze": ["Харків-1 (Бушуй Олексій, Гетманцев Олександр, Котляр Юрій, Лещенко Юрій)"],
        },
        "medalsWomen": {
            "gold": "Маріуполь (Чумак Ганна, Грушовець Ольга)",
            "silver": "Дніпро-Київ (Моісеєнко Віталіна, Ледєньова Наталя)",
            "bronze": ["Черкаси (Харченко Тетяна, Соколова Вікторія)"],
        },
    },
    "26.05.2018": {
        "medals": {
            "gold": "Харків-1 (Бушуй Олексій, Гетманцев Олександр, Ковальов Дмитро, Котляр Юрій)",
            "silver": "Кривий Ріг-1 (Мамика Олександр, Васюра Андрій, Омельченко Владислав, Невінчаний Едуард)",
            "bronze": ["Київ-1 (Усик Артем, Ледєньов Вадим, Григоренко Денис, Ромашко Віталій)"],
        },
        "medalsWomen": {
            "gold": "Маріуполь (Чумак Ганна, Грушовець Ольга)",
            "silver": "Київ (Ледєньова Наталя, Ільясова Альбіна)",
            "bronze": ["Харків (Марфола Оксана, Борисова Марина)"],
        },
    },
    "03.06.2017": {
        "medals": {
            "gold": "Кривбас-1 (Мамика Олександр, Васюра Андрій, Омельченко Владислав, Невінчаний Едуард)",
            "silver": "Київ-1 (Усик Артем, Григоренко Денис, Ромашко Віталій, Ледєньов Вадим)",
            "bronze": ["Черкаси-1 (Марченко Денис, Зольдін Фелікс, Зарайський Володимир, Пальонний Ігор)"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна/Анісімова Світлана",
            "silver": "Чумак Ганна/Грушовець Ольга",
            "bronze": ["Ледєньова Наталя/Павлюченко Марина"],
        },
    },
    "28.05.2016": {
        "medals": {
            "gold": "Київ-1 (Усик Артем, Григоренко Денис, Ромашко Віталій, Ледєньов Вадим)",
            "silver": "Кривий Ріг-1 (Мамика Олександр, Васюра Андрій, Омельченко Владислав, Луков Олександр)",
            "bronze": ["Черкаси-2 (Марченко Денис, Зольдін Фелікс, Зарайський Володимир, Анісімов Денис)"],
        },
        "medalsWomen": {
            "gold": "Київ-1 (Ледєньова Наталія, Редзюк Тетяна)",
            "silver": "Маріуполь (Чумак Ганна, Грушовець Ольга)",
            "bronze": ["Дніпро (Моісеєнко Віталіна, Анісімова Світлана)"],
        },
    },
    "31.05.2015": {
        "medals": {
            "gold": "Черкаси-2 (Марченко, Зольдін, Зарайський, Бублик)",
            "silver": "Кривий Ріг-1 (Омельченко, Шинкаренко, Васюра, Луков)",
            "bronze": ["Волинь (Шмідке, Швець, Бадира, Волчук)"],
        },
        "medalsWomen": {
            "gold": "Черкаси-1 (Соколова, Харченко)",
            "silver": "Дніпропетровськ (Моісеєнко, Потан)",
            "bronze": ["Маріуполь (Чумак, Анісімова)"],
        },
    },
    "06.12.2019": {
        "medals": {
            "gold": "Омельченко Владислав/Марфола Оксана",
            "silver": "Усик Артем/Ледєньова Наталя",
            "bronze": ["Мамика Олександр/Харченко Тетяна"],
        },
        "medalsWomen": None,
    },
    "18.05.2024": {
        "medals": {
            "gold": "United Ukraine (Микитин Ярослав, Залевський Володимир, Ледєньов Вадим, Мелашенко Владислав)",
            "silver": "Черкаси-1 (Чепурний Едуард, Соколов Володимир, Подольський Святослав, Пальонний Ігор)",
            "bronze": ["Харків-1 (Могилевський Руслан, Бутов Олександр, Пекарук Ілля, Василевський Володимир)"],
        },
        "medalsWomen": {
            "gold": "Черкаси (Харченко Тетяна, Гончаренко Інна)",
            "silver": "Одеса - Кривий Ріг (Ільясова Альбіна, Терещенко Любов)",
            "bronze": ["Харків - Татарів (Соколова Вікторія, Лановенко Ксенія)"],
        },
    },
    "02.12.2023": {
        "medals": {
            "gold": "Омельченко Владислав/Терещенко Любов",
            "silver": "Могилевський Руслан/Лановенко Ксенія",
            "bronze": ["Луков Олександр/Лукова Вікторія", "Залевський Володимир/Ігнатюк Наталія"],
        },
        "medalsWomen": None,
    },
    "04.12.2022": {
        "medals": {
            "gold": "Омельченко Владислав/Баширов Ярослав",
            "silver": "Залевський Володимир/Бейреш Ласло",
            "bronze": ["Могилевський Руслан/Пекарук Ілля", "Аверін Сергій/Агневщиков Юрій"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Ледєньова Наталія",
            "bronze": ["Соколова Вікторія"],
        },
    },
    "07.12.2019": {
        "medals": {
            "gold": "Бушуй Олексій",
            "silver": "Омельченко Владислав",
            "bronze": ["Мамика Олександр"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Чумак Ганна",
            "bronze": ["Харченко Тетяна"],
        },
    },
    "09.12.2018": {
        "medals": {
            "gold": "Усик Артем",
            "silver": "Омельченко Владислав",
            "bronze": ["Бушуй Олексій"],
        },
        "medalsWomen": {
            "gold": "Чумак Ганна",
            "silver": "Павлюченко Марина",
            "bronze": ["Марфола Оксана"],
        },
    },
    "25.11.2017": {
        "medals": {
            "gold": "Бушуй Олексій/Павлюченко Марина",
            "silver": "Бурмака Павло/Моісеєнко Віталіна",
            "bronze": ["Мигалюк Родіон/Саїнчук Тетяна"],
        },
        "medalsWomen": None,
    },
    "26.11.2017": {
        "medals": {
            "gold": "Усик Артем",
            "silver": "Невінчаний Едуард",
            "bronze": ["Омельченко Владислав"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Чумак Ганна",
            "bronze": ["Ледєньова Наталія"],
        },
    },
    "27.11.2016": {
        "medals": {
            "gold": "Бушуй Олексій",
            "silver": "Григоренко Денис",
            "bronze": ["Васюра Андрій"],
        },
        "medalsWomen": {
            "gold": "Анісімова Світлана",
            "silver": "Чумак Ганна",
            "bronze": ["Редзюк Тетяна"],
        },
    },
    "29.11.2015": {
        "medals": {
            "gold": "Пасічніченко Артем",
            "silver": "Усик Артем",
            "bronze": ["Омельченко Владислав"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Марфола Оксана",
            "bronze": ["Редзюк Тетяна"],
        },
    },
    "26.11.2016": {
        "medals": {
            "gold": "Усик Артем/Ледєньова Наталія",
            "silver": "Мамика Олександр/Чумак Ганна",
            "bronze": ["Григоренко Денис/Редзюк Тетяна"],
        },
        "medalsWomen": None,
    },
    "28.11.2015": {
        "medals": {
            "gold": "Усик Артем/Ледєньова Наталя",
            "silver": "Мамика Олександр/Чумак Ганна",
            "bronze": ["Бурмака Павло/Моісеєнко Віталіна"],
        },
        "medalsWomen": None,
    },
    "30.06.2018": {
        "medals": {
            "gold": "Мамика Олександр",
            "silver": "Залевський Володимир",
            "bronze": ["Рудковський Василь"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Грушовець Ольга",
            "bronze": ["Чумак Ганна"],
        },
    },
    "16.03.2019": {
        "medals": {
            "gold": "Усик Артем",
            "silver": "Омельченко Владислав",
            "bronze": ["Мамика Олександр"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Ледєньова Наталя",
            "bronze": ["Марфола Оксана"],
        },
    },
    "17.03.2018": {
        "medals": {
            "gold": "Усик Артем",
            "silver": "Омельченко Владислав",
            "bronze": ["Бушуй Олексій"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Ледєньова Наталя",
            "bronze": ["Грушовець Ольга"],
        },
    },
    "18.03.2017": {
        "medals": {
            "gold": "Усик Артем",
            "silver": "Васюра Андрій",
            "bronze": ["Мамика Олександр"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Анісімова Світлана",
            "bronze": ["Чумак Ганна"],
        },
    },
    "15.10.2016": {
        "medals": {
            "gold": "Бушуй Олексій",
            "silver": "Мамика Олександр",
            "bronze": ["Григоренко Денис"],
        },
        "medalsWomen": {
            "gold": "Чумак Ганна",
            "silver": "Редзюк Тетяна",
            "bronze": ["Моісеєнко Віталіна"],
        },
    },
    "17.10.2015": {
        "medals": {
            "gold": "Мамика Олександр",
            "silver": "Омельченко Владислав",
            "bronze": ["Усик Артем"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Чумак Ганна",
            "bronze": ["Грушовець Ольга"],
        },
    },
    "03.09.2011": {
        "medals": {
            "gold": "Лец Дмитро/Євменєнков Максим",
            "silver": "Шинкаренко Василь/Десинський Максим",
            "bronze": ["Варивода Володимир/Новак Едуард"],
        },
        "medalsWomen": {
            "gold": "Луговська Людмила/Скакун Ірина",
            "silver": "Дзюбій Юлія/Тихобаєва Вікторія",
            "bronze": ["Перетятько Тетяна/Гавриш Ольга", "Калаур Інна/Потан Лілія"],
        },
    },
    "15.10.2011": {
        "medals": {
            "gold": "Лец Дмитро",
            "silver": "Невінчаний Едуард",
            "bronze": ["Брежнєв Олександр"],
        },
        "medalsWomen": {
            "gold": "Новікова Зоя",
            "silver": "Качура Катерина",
            "bronze": ["Перетятько Тетяна"],
        },
    },
    "30.01.2011": {
        "medals": {
            "gold": "Мелашенко Владислав",
            "silver": "Мамика Олександр",
            "bronze": ["Десинський Максим"],
        },
        "medalsWomen": {
            "gold": "Білодід Крістіна",
            "silver": "Калаур Інна",
            "bronze": ["Моісеєнко Віталіна"],
        },
    },
    "31.01.2009": {
        "medals": {
            "gold": "Воронін Олександр",
            "silver": "Чик Едуард",
            "bronze": ["Кобзар Ігор"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Павлюченко Марина",
            "bronze": ["Федоришина Алла"],
        },
    },
    "22.05.2010": {
        "medals": {
            "gold": "Ужгород (Мелашенко Владислав, Чік Едуард, Любих Сергій, Острадчук Вадим)",
            "silver": "Київ-1 (Ледєньов Вадим, Скарлат Дмитро, Квасков Павло, Ромашко Віталій)",
            "bronze": ["СК Богатир (Мамика Олександр, Дихтяр Вадим, Шинкаренко Василь, Варивода Володимир)"],
        },
        "medalsWomen": None,
    },
    "11.04.2021": {
        "medals": {
            "gold": "Бушуй Олексій",
            "silver": "Усик Артем",
            "bronze": ["Омельченко Владислав", "Кисельов Дмитро"],
        },
        "medalsWomen": None,
    },
    "10.04.2021": {
        "medals": {
            "gold": "Усик Артем/Усик Сергій",
            "silver": "Омельченко Владислав/Солтис Андрій",
            "bronze": ["Борисова Марина/Бушуй Олексій", "Пекарук Ілля/Троценко Артем"],
        },
        "medalsWomen": None,
    },
    "09.08.2020": {
        "medals": {
            "gold": "Бушуй Олексій",
            "silver": "Усик Артем",
            "bronze": ["Усик Сергій", "Закіров Марат"],
        },
        "medalsWomen": None,
    },
    "08.08.2020": {
        "medals": {
            "gold": "Борисова Марина/Бушуй Олексій",
            "silver": "Могилевський Руслан/Гетманцев Олександр",
            "bronze": ["Захарова З./Усик Артем", "Мельтєва Ірина/Усик Сергій"],
        },
        "medalsWomen": None,
    },
    "11.08.2019": {
        "medals": {
            "gold": "Омельченко Владислав",
            "silver": "Мамика Олександр",
            "bronze": ["Бушуй Олексій", "Усик Артем"],
        },
        "medalsWomen": None,
    },
    "10.08.2019": {
        "medals": {
            "gold": "Омельченко Владислав/Мамика Олександр",
            "silver": "Гетманцев Олександр/Могилевський Руслан",
            "bronze": ["Рогов Віталій/Солтис Андрій", "Рудковський Василь/Солтис Микита"],
        },
        "medalsWomen": None,
    },
    "12.08.2018": {
        "medals": {
            "gold": "Пекарук Ілля",
            "silver": "Торський Андрій",
            "bronze": ["Камельков Валерій"],
        },
        "medalsWomen": None,
    },
    "11.08.2018": {
        "medals": {
            "gold": "Клочек Ксенія",
            "silver": "Пекарук Ілля",
            "bronze": ["Торський Андрій"],
        },
        "medalsWomen": None,
    },
    "17.03.2013": {
        "medals": {
            "gold": "Омельченко Владислав",
            "silver": "Невінчаний Едуард",
            "bronze": ["Хімчак Сергій"],
        },
        "medalsWomen": None,
    },
    "03.12.2022": {
        "medals": {
            "gold": "Омельченко Владислав",
            "silver": "Залевський Володимир",
            "bronze": ["Пекарук Ілля", "Мелашенко Владислав"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Ледєньова Наталія",
            "bronze": ["Соколова Вікторія"],
        },
    },
    "03.12.2023": {
        "medals": {
            "gold": "Омельченко Владислав",
            "silver": "Залевський Володимир",
            "bronze": ["Троценко Артем", "Пекарук Ілля"],
        },
        "medalsWomen": {
            "gold": "Терещенко Любов",
            "silver": "Шклярук Наталія",
            "bronze": ["Соколова Вікторія", "Білоус Жанна"],
        },
    },
    "11.11.2023": {
        "medals": {
            "gold": "Омельченко Владислав",
            "silver": "Аверін Сергій",
            "bronze": ["Могилевський Руслан", "Мамика Олександр"],
        },
        "medalsWomen": {
            "gold": "Моісеєнко Віталіна",
            "silver": "Терещенко Любов",
            "bronze": ["Полтавець Світлана", "Шклярук Наталія"],
        },
    },
    "29.07.2023": {
        "medals": {
            "gold": "Омельченко Владислав",
            "silver": "Залевський Володимир",
            "bronze": ["Мелашенко Владислав", "Пекарук Ілля"],
        },
        "medalsWomen": {
            "gold": "Терещенко Любов",
            "silver": "Лановенко Ксенія",
            "bronze": ["Полтавець Світлана", "Шклярук Наталія"],
        },
    },
    "20.05.2023": {
        "medals": {
            "gold": "Харків-1 (Могилевський Руслан, Пекарук Ілля, Шапка Євген, Гетманцев Олександр)",
            "silver": "United Ukraine (Микитин Ярослав, Мелашенко Владислав, Ледєньов Вадим, Залевський Володимир)",
            "bronze": ["Саксагань (Омельченко Владислав, Мамика Олександр, Баширов Ярослав, Десинський Максим)"],
        },
        "medalsWomen": {
            "gold": "Черкаси-1 (Моісеєнко Віталіна, Соколова Вікторія)",
            "silver": "Черкаси-2 (Харченко Тетяна, Осадча Юлія)",
            "bronze": ["Авдіївка-Київ (Пивошенко Олена, Білаш Вікторія)"],
        },
    },
    "11.10.2025": {
        "medals": {
            "gold": "Мамика Олександр",
            "silver": "Рогов Віталій",
            "bronze": ["Могилевський Руслан", "Омельченко Владислав"],
        },
        "medalsWomen": {
            "gold": "Харченко Тетяна",
            "silver": "Терещенко Любов",
            "bronze": ["Лановенко Ксенія", "Гончаренко Інна"],
        },
    },
    "17.05.2025": {
        "medals": {
            "gold": "United West (Микитин Ярослав, Білоус Іван, Григоренко Віктор, Гакман Степан)",
            "silver": "Кривий Ріг (Омельченко Владислав, Рогов Віталій, Дідов Василь, Солтис Микита)",
            "bronze": ["Команда «Київ» (Усик Сергій, Абчугський Олександр, Буханцов Тарас, Аверін Сергій)"],
        },
        "medalsWomen": {
            "gold": "Авдіївка-Київ (Пивошенко Олена, Ледєньова Наталія)",
            "silver": "Одеса-Татарів (Ільясова Альбіна, Білоус Жанна)",
            "bronze": ["Черкаси-1 (Харченко Тетяна, Гончаренко Інна)"],
        },
    },
    "16.02.2025": {
        "medals": {"gold": "Кіфор Ярослав/Ільясова Альбіна", "silver": "Залевський Володимир/Білоус Жанна",
                    "bronze": ["Солтис Микита/Солтис Ганна"]},
        "medalsWomen": None,
    },
    "24.05.2026": {
        "medals": {
            "gold": "Kyiv Bulls (Овсієнко Євген, Бабанін Дмитро, Абчугський Олександр, Ромашко Віталій)",
            "silver": "Кривий Ріг-1 (Омельченко Владислав, Солтис Микита, Рогов Віталій, Дідов Василь)",
            "bronze": ["Харків (Могилевський Руслан, Хімчак Сергій, Хоменко Андрій, Шапка Євген)"],
        },
        "medalsWomen": {
            "gold": "Черкаси-1 (Гончаренко Інна, Лановенко Ксенія, Харченко Тетяна)",
            "silver": "Кривий Ріг-1 (Шклярук Юлія, Терещенко Любов, Шклярук Наталія)",
            "bronze": ["Полтава-Донецьк (Зайцева Інна, Діденко Оксана, Ананенко Альона)"],
        },
    },
    "12.10.2013": {
        "medals": {"gold": "Лец Дмитро", "silver": "Мамика Олександр", "bronze": ["Усик Артем"]},
        "medalsWomen": {"gold": "Чумак Ганна", "silver": "Лопухова Ганна", "bronze": ["Новікова Зоя"]},
    },
    "07.08.2011": {
        "medals": {"gold": "Орєшкін Олександр", "silver": "Костильов Євгеній", "bronze": ["Лец Дмитро"]},
        "medalsWomen": {"gold": "Білодід Христина", "silver": "Пунцуле Аніта", "bronze": ["Стрільченя Христина"]},
    },
    "11.08.2013": {
        "medals": {"gold": "Лец Дмитро", "silver": "Бургелов Олег", "bronze": ["Усик Артем"]},
        "medalsWomen": {"gold": "Лопухова Ганна", "silver": "Чумак Ганна", "bronze": ["Бородуліна Ганна"]},
    },
    "13.10.2013": {
        "medals": {"gold": "Усик Артем", "silver": "Костильов Євгеній", "bronze": ["Титков Олег"]},
        "medalsWomen": {"gold": "Лопухова Ганна", "silver": "Новікова Зоя", "bronze": ["Чумак Ганна"]},
    },
    "14.09.2013": {
        "medals": {"gold": "Костильов Євгеній", "silver": "Мамика Олександр", "bronze": ["Гросов Олексій"]},
        "medalsWomen": {"gold": "Лопухова Ганна", "silver": "Леонова Єлізавета", "bronze": ["Соловьова Юлія"]},
    },
    "15.09.2013": {
        # Пари — переможці це пари гравців
        "medals": {
            "gold": "Лец Дмитро/Титков Олег",
            "silver": "Єрєменко Василій/Гросов Олексій",
            "bronze": ["Омельченко Владислав/Невінчаний Едуард"],
        },
        "medalsWomen": {
            "gold": "Ниценко Ольга/Леонова Єлізавета",
            "silver": "Лопухова Ганна/Гладка Ірина",
            "bronze": ["Шумілова Ірина/Чумак Ганна"],
        },
    },
}


def fill_medals_from_prizes_sheet(tournaments, men_year_data, women_year_data, name_index, canonical_names):
    """Заповнює t['medals']/t['medalsWomen'] з таблиці 'Призери етапів кубків
    ВФД' — критично для турнірів до 2022 року, коли Nakka ще не існувала.
    Зіставлення йде за РОКОМ + НОМЕРОМ ЕТАПУ (не за містом чи назвою — це
    показало себе ненадійним раніше через дрібні розбіжності написання).
    Фронтенд сам віддає перевагу nakkaMedals, якщо вони є, тож тут можна
    сміливо заповнювати medals для ВСІХ турнірів без ризику щось перебити."""

    def stage_key(t):
        name = t.get("name", "")
        if name.strip().upper().startswith("ЧУ") or "чемпіонат україни" in name.lower():
            return "ЧУ"
        m = re.search(r"(\d+)\s*етап", t.get("format", ""), re.IGNORECASE)
        return m.group(1) if m else None

    def podium_to_medals(podium, name_index, canonical_names):
        names = [resolve_name(n, name_index, canonical_names) if n else None for n in podium]
        if not names or not names[0]:
            return None
        return {
            "gold": names[0],
            "silver": names[1] if len(names) > 1 else None,
            "bronze": [n for n in names[2:] if n],
        }

    filled_men = filled_women = 0
    for t in tournaments:
        # Ручні перевизначення — завжди перебивають автоматичне зіставлення,
        # незалежно від того, чи вдалось визначити рік/етап. Перевіряємо їх
        # ПЕРШИМИ, щоб не втратити через "continue" нижче.
        override = MANUAL_MEDAL_OVERRIDES.get((t["date"], t["name"])) or MANUAL_MEDAL_OVERRIDES_BY_DATE.get(t["date"])
        if override:
            t["medals"] = override.get("medals", t.get("medals"))
            t["medalsWomen"] = override.get("medalsWomen", t.get("medalsWomen"))

        try:
            year = int(t["date"].split(".")[-1])
        except (ValueError, IndexError):
            continue
        key = stage_key(t)
        if not key:
            continue

        men_entry = men_year_data.get(year, {}).get(key)
        if men_entry and not t.get("medals"):
            medals = podium_to_medals(men_entry["podium"], name_index, canonical_names)
            if medals:
                t["medals"] = medals
                filled_men += 1

        women_entry = women_year_data.get(year, {}).get(key)
        if women_entry and not t.get("medalsWomen"):
            medals = podium_to_medals(women_entry["podium"], name_index, canonical_names)
            if medals:
                t["medalsWomen"] = medals
                filled_women += 1

    return filled_men, filled_women


def fill_protocol_medals(tournaments):
    """Для турнірів, де досі немає жодних призерів (ні з Nakka, ні з таблиці
    Google Sheets), пробує дістати їх з протоколу (Google Docs), якщо він
    прив'язаний як посилання на цей турнір."""
    filled = 0
    cache = {}
    for t in tournaments:
        if t.get("nakkaMedals"):
            continue  # вже є призери з Nakka — не чіпаємо

        protocol_url = None
        for key in ("tournament", "men", "menAvg", "women", "womenAvg"):
            link = t.get("links", {}).get(key)
            if link and link.get("type") == "protocol_doc":
                protocol_url = link["url"]
                break
        if not protocol_url:
            continue

        if protocol_url not in cache:
            cache[protocol_url] = fetch_protocol_podium(protocol_url)
            time.sleep(0.05)
        podium = cache[protocol_url]
        if not podium:
            continue

        t["medals"] = {
            "gold": podium[0] if len(podium) > 0 else None,
            "silver": podium[1] if len(podium) > 1 else None,
            "bronze": [n for n in podium[2:] if n],
        }
        filled += 1
    return filled


# ---------------------------------------------------------------------------
# YouTube — останні відео каналу федерації через публічну RSS-стрічку
# (не потребує API-ключа; сторінка каналу/videos блокує ботів, а цей
# фід — ні).
# ---------------------------------------------------------------------------
def fetch_latest_youtube_videos(count=4):
    import xml.etree.ElementTree as ET

    url = f"https://www.youtube.com/feeds/videos.xml?channel_id={YOUTUBE_CHANNEL_ID}"
    ns = {
        "atom": "http://www.w3.org/2005/Atom",
        "yt": "http://www.youtube.com/xml/schemas/2015",
        "media": "http://search.yahoo.com/mrss/",
    }
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (vfd-darts-sync)"})
        with urllib.request.urlopen(req, timeout=20) as resp:
            xml_text = resp.read().decode("utf-8")
        root = ET.fromstring(xml_text)
        videos = []
        for entry in root.findall("atom:entry", ns)[:count]:
            video_id_el = entry.find("yt:videoId", ns)
            title_el = entry.find("atom:title", ns)
            published_el = entry.find("atom:published", ns)
            thumb_el = entry.find(".//media:thumbnail", ns)
            if video_id_el is None or title_el is None:
                continue
            video_id = video_id_el.text
            videos.append({
                "id": video_id,
                "title": title_el.text,
                "published": (published_el.text or "")[:10] if published_el is not None else "",
                "thumbnail": thumb_el.get("url") if thumb_el is not None
                             else f"https://i.ytimg.com/vi/{video_id}/mqdefault.jpg",
            })
        return videos
    except Exception as e:
        print(f"  YouTube RSS fetch failed: {e}")
        return []


# ---------------------------------------------------------------------------
# Telegram — новини від інформаційного партнера ВФД (Fullbull). Канал пише
# і про світовий дартс, тому фільтруємо: лишаємо тільки пости, де є явний
# український сигнал (слово "україн", ВФД/УДЛ/ЗУДЛ, наше місто-господар,
# або ім'я гравця з нашої ж бази).
# ---------------------------------------------------------------------------
UKRAINE_SIGNAL_WORDS = ["україн", "вфд", " удл", "зудл", "чемпіонат україни", "кубок україни"]


def is_ukraine_relevant(text, known_names, known_cities):
    t = text.lower()
    if any(w in t for w in UKRAINE_SIGNAL_WORDS):
        return True
    if any(city.lower() in t for city in known_cities):
        return True
    if any(name.lower() in t for name in known_names if len(name) > 3):
        return True
    return False


def strip_html_tags(fragment):
    text = re.sub(r"<br\s*/?>", "\n", fragment)
    text = re.sub(r"<[^>]+>", "", text)
    text = _html_module.unescape(text).strip()
    # Голі посилання не несуть змісту на карточці (Nakka/YouTube лінки в
    # тексті поста) — прибираємо їх, лишаючи чистий текст новини.
    text = re.sub(r"https?://\S+", "", text)
    # Прибираємо порожні рядки, що лишились після видалення посилань
    text = re.sub(r"\n\s*\n+", "\n", text)
    text = re.sub(r"[ \t]+", " ", text)
    return text.strip()


def fetch_telegram_news(channel, known_names, known_cities, limit=6):
    url = f"https://t.me/s/{channel}"
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (vfd-darts-sync)"})
        with urllib.request.urlopen(req, timeout=20) as resp:
            page = resp.read().decode("utf-8", errors="ignore")
    except Exception as e:
        print(f"  Telegram fetch failed: {e}")
        return []

    blocks = re.split(r'(?=<div class="tgme_widget_message_wrap)', page)
    posts = []
    for block in blocks:
        text_m = re.search(r'<div class="tgme_widget_message_text[^"]*"[^>]*>(.*?)</div>', block, re.DOTALL)
        date_m = re.search(r'<time[^>]*datetime="([^"]+)"', block)
        link_m = re.search(r'href="(https://t\.me/[a-zA-Z0-9_]+/\d+)"', block)
        photo_m = re.search(r"tgme_widget_message_photo_wrap[^\"']*[\"'][^>]*background-image:url\('([^']+)'\)", block)
        if not text_m or not date_m:
            continue
        text = strip_html_tags(text_m.group(1))
        if not text:
            continue
        posts.append({
            "text": text[:500],
            "date": date_m.group(1)[:10],
            "url": link_m.group(1) if link_m else f"https://t.me/{channel}",
            "photo": photo_m.group(1) if photo_m else None,
        })

    posts.reverse()  # t.me/s/ віддає від найстарішого до найновішого
    filtered = [p for p in posts if is_ukraine_relevant(p["text"], known_names, known_cities)]
    return filtered[:limit]


# ---------------------------------------------------------------------------
# Поточний рейтинг (живий, змінюється протягом сезону). На відміну від 15
# архівних сезонів "Кубка України", тут нам потрібна ІСТОРІЯ — як позиція
# кожного гравця змінювалась із часом. Документ сам показує лише різницю
# з попередньої версії (+2/-1/=), тому щоб намалювати графік, ми самі
# накопичуємо щоденні знімки в окремий файл, який зберігається в репозиторії
# між запусками (на відміну від data.json, який завжди перезаписується
# повністю, rating_history.json НАРОЩУЄТЬСЯ з кожним запуском).
# ---------------------------------------------------------------------------
def fetch_current_rating(url, name_index, canonical_names):
    rows = fetch_csv(url)
    parsed = parse_ratings_sheet(rows)
    if not parsed:
        return None
    for row in parsed["rows"]:
        row["name"] = resolve_name(row["name"], name_index, canonical_names)
    return parsed


def load_rating_history(path):
    try:
        with open(path, encoding="utf-8") as f:
            return json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        return {"men": {}, "women": {}}


def append_rating_snapshot(history, gender, snapshot_rows):
    """Додає сьогоднішній знімок рейтингу. Якщо синхронізація вже
    запускалась сьогодні (напр. вручну кілька разів), сьогоднішній запис
    просто перезаписується — не множимо однакові дні."""
    today = datetime.now(timezone.utc).date().isoformat()
    history.setdefault(gender, {})[today] = [
        {"name": r["name"], "rank": r["rank"], "total": r["total"]}
        for r in snapshot_rows
        if r.get("total") is not None
    ]


def main():
    print("Fetching tournaments CSV...")
    t_rows = fetch_csv(TOURNAMENTS_CSV_URL)
    print(f"  {len(t_rows)} raw rows")

    print("Fetching men's prizes CSV...")
    men_rows = fetch_csv(PRIZES_MEN_CSV_URL)
    print(f"  {len(men_rows)} raw rows")

    women_rows = []
    try:
        print("Fetching women's prizes CSV...")
        women_rows = fetch_csv(PRIZES_WOMEN_CSV_URL)
        print(f"  {len(women_rows)} raw rows")
    except Exception as e:
        print(f"  WARNING: could not fetch women's sheet ({e}); continuing without it")

    tournaments = parse_tournaments(t_rows)
    print(f"Parsed {len(tournaments)} tournaments")

    men_year_data, _sheet_men_aggregate = parse_prizes(men_rows)
    print(f"Parsed men's prize data for {len(men_year_data)} years")

    women_year_data = {}
    if women_rows:
        women_year_data, _sheet_women_aggregate = parse_prizes(women_rows)
        print(f"Parsed women's prize data for {len(women_year_data)} years")

    # Рахуємо медальний залік самі з даних подіумів, а не з ручної таблиці
    # внизу аркуша (там могли закрастись помилки при ручному підбитті).
    # Це єдине призначення таблиці "Призери етапів кубків ВФД" на сайті —
    # її записи більше НЕ намагаємось зіставляти з конкретними турнірами
    # (це виявилось занадто крихким через розбіжності в нумерації/містах).
    men_aggregate = build_leaderboard_from_podiums(men_year_data)
    women_aggregate = build_leaderboard_from_podiums(women_year_data)
    print(f"Computed leaderboard ourselves: {len(men_aggregate)} men, {len(women_aggregate)} women")

    for t in tournaments:
        t["medals"] = None
        t["medalsWomen"] = None

    # Єдина база відомих імен (канонічно — "Прізвище Ім'я", українською),
    # побудована з нашого найнадійнішого джерела — самостійно порахованого
    # медального заліку. Використовується для уніфікації імен всюди далі:
    # в рейтингах сезону і в статистиці Nakka — де прізвища часто пишуть
    # по-різному (одруківки, порядок слів, українська/російська форма).
    name_sources = []
    name_sources.extend(p["name"] for p in men_aggregate)
    name_sources.extend(p["name"] for p in women_aggregate)
    name_index = build_name_index(name_sources)
    canonical_names = build_canonical_names(name_sources)
    print(f"  Built name index with {len(name_index)} known surnames, "
          f"{len(canonical_names)} canonical full names")

    filled_m, filled_w = fill_medals_from_prizes_sheet(
        tournaments, men_year_data, women_year_data, name_index, canonical_names
    )
    print(f"  Filled medals from prizes sheet: {filled_m} men, {filled_w} women "
          f"(переважно турніри до 2022 року, без Nakka)")

    print("Fetching Champions of Ukraine registry (окрема вкладка, усі категорії ЧУ)...")
    try:
        champions_rows = fetch_csv(CHAMPIONS_CSV_URL)
        champions_records = parse_champions_sheet(champions_rows, name_index, canonical_names)
        print(f"  Parsed {len(champions_records)} champions records")
    except Exception as e:
        print(f"  WARNING: champions sheet fetch failed ({e}), skipping")
        champions_records = []

    print("Fetching player bios (заповнюється самими гравцями)...")
    try:
        bio_rows = fetch_csv(BIO_CSV_URL)
        player_bios = parse_bio_sheet(bio_rows, name_index, canonical_names)
        print(f"  Parsed bios for {len(player_bios)} players")
    except Exception as e:
        print(f"  WARNING: bio sheet fetch failed ({e}), skipping")
        player_bios = {}

    print("Fetching season ratings (Кубок України, all tabs)...")
    ratings = build_ratings(RATINGS_SOURCES_PATH, name_index, canonical_names)
    print(f"Parsed {len(ratings)} rating seasons")

    print("Fetching real Nakka tournament stats (this may take a few minutes)...")
    known_women = {p["name"] for p in women_aggregate}
    known_men = {p["name"] for p in men_aggregate}
    nakka_player_records, nakka_match_records, nakka_h2h_records = enrich_with_nakka(tournaments, name_index, canonical_names, known_women, known_men)
    apply_manual_medal_overrides_final(tournaments)
    print(f"Collected {len(nakka_player_records)} player-tournament stat rows from Nakka")

    # Протоколи (Google Docs) для турнірів до Nakka НЕ вмикаємо автоматично:
    # більшість із них — це повний розпис матчів по раундах, а не готова
    # таблиця місць, тож автопарсинг короткого опису дає ненадійні (іноді
    # просто неправильні) результати. Функції parse_protocol_description /
    # fetch_protocol_podium / fill_protocol_medals лишаються в коді нижче —
    # повернемось до цього, коли протоколи будуть уніфіковані в один формат.
    # protocol_count = fill_protocol_medals(tournaments)

    print("Fetching current-season live rating (men + women)...")
    try:
        current_rating_men = fetch_current_rating(CURRENT_RATING_MEN_URL, name_index, canonical_names)
    except Exception as e:
        print(f"  WARNING: men's current rating fetch failed ({e}), skipping")
        current_rating_men = None
    try:
        current_rating_women = fetch_current_rating(CURRENT_RATING_WOMEN_URL, name_index, canonical_names)
    except Exception as e:
        print(f"  WARNING: women's current rating fetch failed ({e}), skipping")
        current_rating_women = None
    print(f"  Men: {len(current_rating_men['rows']) if current_rating_men else 0} players, "
          f"Women: {len(current_rating_women['rows']) if current_rating_women else 0} players")

    rating_history = load_rating_history(RATING_HISTORY_PATH)
    if current_rating_men:
        append_rating_snapshot(rating_history, "men", current_rating_men["rows"])
    if current_rating_women:
        append_rating_snapshot(rating_history, "women", current_rating_women["rows"])
    with open(RATING_HISTORY_PATH, "w", encoding="utf-8") as f:
        json.dump(rating_history, f, ensure_ascii=False, indent=1)
    total_snapshots = sum(len(v) for v in rating_history.values())
    print(f"  rating_history.json now has {total_snapshots} daily snapshots total")

    print("Fetching latest YouTube videos...")
    youtube_videos = fetch_latest_youtube_videos(3)
    print(f"  Got {len(youtube_videos)} videos")

    print("Fetching Telegram news (Fullbull)...")
    ukr_cities = list({t["city"] for t in tournaments if t.get("city")})
    telegram_news = fetch_telegram_news(TELEGRAM_CHANNEL, canonical_names, ukr_cities, limit=6)
    print(f"  Got {len(telegram_news)} Ukraine-relevant posts")

    print("Comparing live data against curated player list (auto-adding genuinely new players)...")
    live_names = set()
    for r in nakka_player_records:
        live_names.add(r["name"])
    for p in men_aggregate:
        live_names.add(p["name"])
    for p in women_aggregate:
        live_names.add(p["name"])
    for season in ratings.values():
        for row in season.get("rows", []):
            if row.get("name"):
                live_names.add(row["name"])
    for r in champions_records:
        for n in r["names"]:
            live_names.add(n)
    if current_rating_men:
        for row in current_rating_men["rows"]:
            if row.get("name"):
                live_names.add(row["name"])
    if current_rating_women:
        for row in current_rating_women["rows"]:
            if row.get("name"):
                live_names.add(row["name"])

    curated_set = set(CURATED_PLAYER_NAMES)
    genuinely_new = sorted(n for n in live_names if n and n not in curated_set and not is_pair_or_team_name(n))
    final_player_names = sorted(curated_set | set(genuinely_new), key=str.lower)
    if genuinely_new:
        print(f"  Знайдено {len(genuinely_new)} нових гравців (яких немає у перевіреному списку):")
        for n in genuinely_new[:30]:
            print(f"    + {n}")
        if len(genuinely_new) > 30:
            print(f"    ...та ще {len(genuinely_new) - 30}")
        print("  (Це не пройшло ручної перевірки — раз на якийсь час попросіть новий Excel-експорт для звірки.)")
    else:
        print("  Нових гравців не знайдено — перевірений список і далі актуальний.")

    data = {
        "meta": {
            "lastUpdated": datetime.now(timezone.utc).isoformat(),
            "tournamentsCount": len(tournaments),
        },
        "tournaments": tournaments,
        "leaderboard": men_aggregate,
        "leaderboardWomen": women_aggregate,
        "ratings": ratings,
        "nakkaPlayerStats": nakka_player_records,
        "nakkaMatchStats": nakka_match_records,
        "nakkaH2HStats": nakka_h2h_records,
        "currentRatingMen": current_rating_men,
        "currentRatingWomen": current_rating_women,
        "youtubeVideos": youtube_videos,
        "telegramNews": telegram_news,
        "championsRecords": champions_records,
        "playerBios": player_bios,
        "curatedPlayerNames": final_player_names,
    }

    with open(OUTPUT_PATH, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=1)
    print(f"Wrote {OUTPUT_PATH}")


if __name__ == "__main__":
    main()
